...
export async function generateMetadata({ params }) {
  ...
  if (!o) return { title: "Post Not Found" };
  return { title: o.title };
}

export default async function DiscussionPostPage({ params }) {
  const { id } = await params;
  const o = await getOpinionById(id);
  if (!o) notFound();
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          &laquo; All Discussion
        </Link>
        {admin && <DeleteOpinionButton id={o.id} />}
      </div>

      <p className="mt-8 text-xs text-gray-400">
        Posts are submitted by readers and do not represent HSPN or the
        Longbardi League.
      </p>
    </article>
  );
