...
// Helmet blast. Damage and shove both fall off linearly to nothing at BLAST_R,
// so a helmet levels the glass beside it, staggers wood, and only dents stone
// it is sitting right against.
export const BLAST_R = 140;
const BLAST_DMG = 150;
const BLAST_PUSH = 1400;
const BLAST_FADE = 0.35; // seconds the shockwave ring stays on screen

export const SLING = { x: 152, y: GROUND_Y - 150 };
export const BALL_R = 15;
export const MAX_PULL = 132;
export const POWER = 10.5;
...
function fireball(w, body) {
  w.blasts.push({ x: body.x, y: body.y, r: BLAST_R, t: 0, max: BLAST_FADE });
  w.shake = Math.min(26, w.shake + 15);
  for (let i = 0; i < 32; i++) {
    const a = Math.random() * Math.PI * 2;
...
// One death, in one place, so a block killed by an impact and one killed by a
// blast behave identically. Helmets go on the queue instead of exploding here,
// which is what lets chains resolve without recursing.
function kill(w, body, queue) {
  if (body.dead) return;
  body.dead = true;
  w.shake = Math.min(18, w.shake + (body.kind === "star" ? 10 : 5));
  burst(w, body);
  if (body.kind === "helmet") queue.push(body);
}

function detonate(w, bomb, queue) {
  fireball(w, bomb);
  const caught = w.ball ? [...w.blocks, w.ball] : w.blocks;
  for (const b of caught) {
    if (b === bomb || b.dead || b.inv === 0) continue;
    const dx = b.x - bomb.x;
    const dy = b.y - bomb.y;
    const d = Math.hypot(dx, dy);
    ...
    b.vx += ux * BLAST_PUSH * f * b.inv;
    b.vy += uy * BLAST_PUSH * f * b.inv - 60 * f; // a little lift, always
    if (b.hp === Infinity) continue;
    b.hp -= BLAST_DMG * f;
    if (b.hp <= 0) kill(w, b, queue);
  }
}
...
export function step(w) {
  ...
  for (const [a, b, v] of hits) {
    ...
    for (const [self, other] of [
      ...
      if (self.hp <= 0) kill(w, self, fuse);
    }
  }
  while (fuse.length) detonate(w, fuse.shift(), fuse);
  ...
  for (const b of w.blasts) b.t += STEP;
  w.blasts = w.blasts.filter((b) => b.t < b.max);
  if (w.shake > 0) w.shake = Math.max(0, w.shake - 40 * STEP);

  if (w.ball) {
    const b = w.ball;
    const still = Math.hypot(b.vx, b.vy) < 40;
    const gone = b.x > WORLD_W + 120 || b.y > WORLD_H + 200;
    w.settleTimer = still || gone ? w.settleTimer + STEP : 0;
    if (gone || w.settleTimer > 0.6) {
      w.ball = null;
      w.settleTimer = 0;
    }
  }
