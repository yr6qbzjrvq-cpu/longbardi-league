...
// ---- cards in flight ---------------------------------------
// The dealing animation (m20). The choreography lives in
// dealAnim.js; this module only knows how to paint one card
// mid-trip. A "slide" leaves the shoe small and slightly
// turned and eases into its fan slot; a "flip" happens in
// place — the hole card narrowing to an edge as its back, then
// widening as its face.

export function shoeSpot(VW) {
  // Top right, low enough to clear the HTML "Watch Room"
  // button that floats over the canvas corner.
  return { x: VW - 44, y: VH * 0.135 };
}

function drawShoe(ctx, VW, P) {
  const { x, y } = shoeSpot(VW);
  // the wedge of the shoe, wide end toward the felt
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(-0.08);
  rr(ctx, -26, -19, 52, 38, 7);
  paint(ctx, P.rail, 2.5);
  rr(ctx, -26, -19, 52, 12, 6);
  paint(ctx, P.railHi, 1.8);
  // the next card peeking out of the throat
  rr(ctx, -15, -6, 30, 20, 4);
  paint(ctx, P.cardBack, 1.6);
  ctx.restore();
}
...
function drawFlight(ctx, card, dx, dy, w, h, P, flight, VW) {
  if (flight.kind === "flip") {
    // Turning over where it lies: back shrinking to a sliver,
    // then face growing out of it.
    const p = Math.max(0, Math.min(1, flight.p));
    const sx = Math.max(0.04, Math.abs(1 - 2 * p));
    ctx.save();
    ctx.translate(dx + w / 2, dy + h / 2);
    ctx.scale(sx, 1);
    if (p < 0.5) drawCardBack(ctx, -w / 2, -h / 2, w, h, P);
    else drawCard(ctx, card, -w / 2, -h / 2, w, h, P);
    ctx.restore();
    return;
  }
  const e = easeOut(flight.p);
  const s = shoeSpot(VW);
  const cx = s.x + (dx + w / 2 - s.x) * e;
  const cy = s.y + (dy + h / 2 - s.y) * e;
  const scale = 0.82 + 0.18 * e;
  ctx.save();
  ctx.translate(cx, cy);
  ctx.rotate((1 - e) * -0.3);
  ctx.scale(scale, scale);
  drawCard(ctx, card, -w / 2, -h / 2, w, h, P);
  ctx.restore();
}
...
function drawHand(ctx, hand, cx, y, cwMax, budget, P, opts) {
  ...
  // While the deal animation runs, only the cards that have
  // landed are painted; the one in flight rides in from the
  // shoe toward its slot in the fan. The fan is laid out for
  // the FULL hand from the start, so nothing shuffles sideways
  // as cards arrive — they just fill their places.
  const deal = o.deal || null;
  for (let i = 0; i < cards.length; i += 1) {
    const flying = deal && deal.flight && deal.flight.index === i;
    if (deal && i >= deal.show && !flying) break;
    if (flying) drawFlight(ctx, cards[i], x, y, cw, ch, P, deal.flight, o.VW || 980);
    else drawCard(ctx, cards[i], x, y, cw, ch, P);
    x += step;
  }
  if (cards.length === 0) {
  ...
  // total badge. Its space is reserved even while cards are
  // still in the air (so the chip stack does not jump when it
  // appears), but a total is never SHOWN for cards the deal
  // animation has not landed yet — that would spoil the ride.
  let bottom = y + ch;
  if (cards.length > 0) {
    const settled = o.settled !== false;
...
export function drawTableView(ctx, opts) {
  const { w, h, dpr = 1, theme = "light", table, selfId, now = Date.now(), anim = null } = opts;
  ...
  const dealerSettled = !anim || anim.dealerSettled();
  {
    // The hole card holds the last slot in the fan; during the
    // deal animation it arrives face down like any other card,
    // and at the reveal it flips over where it lies.
    const n = dCards.length + (dealer.hole ? 1 : 0);
    const deal = anim ? anim.dealer(n) : null;
    const fan = fanLayout(dealerCw, n, VW * 0.58);
    const dch = fan.cw * 1.42;
    let x = cxm - fan.totalW / 2;
    for (let i = 0; i < n; i += 1) {
      const card = i < dCards.length ? dCards[i] : null; // null = the hole
      const flying = deal && deal.flight && deal.flight.index === i;
      if (deal && i >= deal.show && !flying) break;
      if (flying) drawFlight(ctx, card, x, dealerY, fan.cw, dch, P, deal.flight, VW);
      else if (card) drawCard(ctx, card, x, dealerY, fan.cw, dch, P);
      else drawCardBack(ctx, x, dealerY, fan.cw, dch, P);
      x += fan.step;
    }
    if (n > 0 && !dealer.hole && dealerSettled) {
  ...
  for (const slot of slots) {
    ...
    for (let hi = 0; hi < n; hi += 1) {
      ...
      const laid = drawHand(ctx, hand, hx, slot.cy, slot.cw * shrink, per, P, {
        active,
        deal: anim ? anim.hand(slot.i, hi, (hand.cards || []).length) : null,
        settled: !anim || anim.handSettled(slot.i, hi),
        VW,
      });
    ...
    // result badge after a hand — held back until the last
    // animated card has landed, so the dealer finishes the
    // story before the payout tells the ending
    if (table.phase === "payout" && seat.result && (!anim || anim.allSettled())) {
      const label =
