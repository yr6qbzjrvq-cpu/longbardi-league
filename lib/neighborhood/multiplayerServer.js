...
// username-uniqueness purposes. Heartbeats are SENT every 30s
// — but "sent" is a promise the browser does not keep.
//
// WHY THIS IS 3.5 MINUTES AND NOT 75 SECONDS (the background
// prune bug). A hidden tab's setInterval is throttled by
// Chrome: measured on Austin's own backgrounded game tab, a
// nominal 5s timer fired at 5,5,5,5,5,27 seconds, and once a
// tab has been hidden ~5 minutes "intensive throttling" clamps
// every timer to at most one wake-up per MINUTE. So a 30s
// heartbeat really arrives every 60-160s while the tab is in
// the background.
//
// The old 75s window was therefore under the floor: it needed
// two heartbeats to land inside a minute and a quarter, which
// a backgrounded tab simply cannot do. The row got deleted out
// from under a live player, their character vanished for
// everyone, and the client quietly rejoined — which is why
// Austin's row kept coming back with a fresh created_at.
//
// Backgrounded is not an edge case here, it is the NORMAL
// state of a broadcaster: the whole point of sharing a screen
// is that you are looking at the other window. So the window
// has to clear Chrome's worst case with room to spare rather
// than race it. 210s survives three consecutive throttled
// beats. Clean exits do not rely on it at all — /leave fires
// with keepalive on tab close — so the only thing a longer
// window costs is that a yanked power cord lingers in the
// roster a couple of minutes instead of one.
export const ACTIVE_WINDOW_MS = 210_000;
...
export async function listActivePlayers(supabase, roomId) {
  const { data, error } = await supabase
    .from(TABLE)
    .select(
      "id, username, avatar, room, x, y, path, path_started_at, last_seen, dance_id, dance_at"
    )
    .eq("room", roomId)
...
export function toWirePlayer(row) {
  return {
    ...
    // milestone 27: a dance has no end time any more, so it is a
    // STATE rather than an event — and a state has to ride the
    // roster, not just the one broadcast that started it. null
    // means "not dancing", which is what heals a client that
    // missed a `dance_stop`, and what lets somebody who walks in
    // late join a dance already in progress on the right beat.
    danceId: row.dance_id || null,
    danceAt: row.dance_at ? Number(row.dance_at) : null,
  };
}

// Topic names live in lib/neighborhood/topics.js so the
// browser bundle can import them without dragging this
// (service-role) module along. Re-exported here because the
// routes have always reached for channelTopic through this
// file.
export { channelTopic, rtcTopic };
...
//
// CHANNEL SECURITY (milestone 10, the go-public flip): the
// room topics are now PRIVATE channels, so every subscribe and
// every client-side publish is checked against RLS policies on
// realtime.messages (supabase/neighborhood_realtime_auth.sql).
// Clients may read the gameplay topic and track presence on
// it; they may NOT publish to it. This call is the only writer,
// and it authenticates with the service-role key, which
// bypasses RLS — so `move`, `chat`, `chat_delete`, `leave` and
// `kicked` can only ever originate from a validated API route.
// `private: true` is REQUIRED here: a private channel does not
// receive messages published as public.
export async function broadcastToRoom(roomId, event, payload) {
  ...
  try {
    await fetch(`${url}/realtime/v1/api/broadcast`, {
      ...
      body: JSON.stringify({
        messages: [{ topic: channelTopic(roomId), event, payload, private: true }],
      }),
    });
  } catch {
