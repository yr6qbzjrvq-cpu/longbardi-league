...
const STINGS = {
  ...
  // The slot reels spinning (milestone 32). Deliberately quiet
  // and mechanical — a soft tick on every eighth, like reels
  // ratcheting past the window, plus a low motor hum. It is the
  // bed the 4-second spin plays over; a win sting or the jackpot
  // fanfare replaces it the instant the reels land.
  slotspin: {
    bpm: 150,
    beats: 4,
    bar(ctx, noiseBuf, dest, t0, bar, spb) {
      for (let b = 0; b < 4; b++) {
        const bt = t0 + b * spb;
        // rapid ratchet ticks, 16ths racing past the window
        ...
        // a steady low motor pulse under it
        tone(ctx, dest, {
          f: hz(33),
          t0: bt,
          dur: spb * 0.62,
  ...
  // A small win (three-of-a-kind or two heads) — a quick, bright
  // three-note chime that rings up and holds. Short: it should
  // be over before the player reaches for the handle again.
  slotwin: {
    bpm: 132,
    beats: 4,
    bar(ctx, noiseBuf, dest, t0, bar, spb) {
      // only ring on the first bar; after that, silence until
      // the sting tears itself down.
      if (bar !== 0) return;
      const run = [72, 76, 79, 84]; // C E G C
      for (let i = 0; i < run.length; i++) {
        key(ctx, dest, {
          f: hz(run[i]),
          t0: t0 + i * spb * 0.28,
      ...
      clap(ctx, noiseBuf, dest, t0 + spb * 0.9, 0.09);
    },
  },

  // THE JACKPOT (milestone 32). Three Austin heads in a row. A
  // full triumphant fanfare: four-on-the-floor kicks, claps on
  // the 2 and 4, a I-IV-V-I brass swell and a cascade of bright
  // bells over the top. Loops for the length the celebration
  // asks for, building each bar.
  jackpot: {
    bpm: 140,
    beats: 4,
    bar(ctx, noiseBuf, dest, t0, bar, spb) {
      const roots = [48, 53, 55, 60]; // C F G C
      const chords = [
        [60, 64, 67], // C
        [60, 65, 69], // F
        [59, 62, 67], // G
      ...
      for (let b = 0; b < 4; b++) {
        const bt = t0 + b * spb;
        kick(ctx, dest, bt, 0.6);
        // open hats on the offbeat
        brush(ctx, noiseBuf, dest, {
          t0: bt + spb * 0.5,
          dur: 0.14,
          vol: 0.05,
        ...
        if (b === 1 || b === 3) clap(ctx, noiseBuf, dest, bt, 0.14);
      }

      // brass swell — the chord, held big, with an octave bass
      ...
      // a cascade of bright bells, ascending, all the way up
      const run = [72, 76, 79, 84, 88, 91, 96];
      for (let i = 0; i < run.length; i++) {
        key(ctx, dest, {
          f: hz(run[i]),
          t0: t0 + spb * (0.5 + i * 0.22),
          dur: spb * 0.9,
          vol: 0.06,
          filterF: 6000,
          attack: 0.004,
...
class RoomMusic {
  ...
  // A single mechanical reel-stop click (milestone 32b). Fired as
  // each reel lands during a spin, it LAYERS over the slotspin bed
  // instead of replacing it: its own throwaway nodes hang off the
  // sting bus, so it never touches this.sting, the room track or the
  // context lifecycle. Silent when muted (the slot overlay only
  // calls it while the speaker toggle is on).
  reelStop() {
    this.unlock();
    const ctx = this.ctx;
    if (!ctx || !this.stingBus) return;
    ...
    src.start(t0);
    src.stop(t0 + 0.16);
    // a short low "clunk" body so it seats like a reel
    const osc = ctx.createOscillator();
    osc.type = "sine";
    ...
    og.gain.linearRampToValueAtTime(0.28, t0 + 0.006);
    og.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.14);
    osc.connect(og);
    og.connect(this.stingBus);
    osc.start(t0);
    osc.stop(t0 + 0.18);
    if (ctx.state === "suspended") ctx.resume().catch(() => {});
  }
