...
//
// Screen share (milestone 9): a SECOND channel per room
// carries the WebRTC handshake for the Mission Control big
// board. The five rtc-* broadcast events are relayed verbatim
// to onSignal, and the connection exposes signal() to send
// them plus presenceIds() so the broadcaster can refuse anyone
// who is not actually standing in the room.
//
...
export async function joinRoom({
  ...
  if (supabase) {
    ...
    rtcChannel = supabase.channel(rtcTopic(roomId), {
    ...
    // The signaling channel joins right behind it. Awaited so
    // that walking into a room already showing a broadcast
    // finds a live channel for the first rtc-hello (a lost one
    // costs a retry beat, not the session).
    await new Promise((resolve) => {
      const failsafe = setTimeout(resolve, 8000);
      let settled = false;
