...
// solo, move(x,y), updateSelf(x,y), detach(), leave() }. Throws with
// err.code "name_taken" / "room_full" / "bad_name" when the
// join is rejected.
export async function joinRoom({ roomId, player, entry, onRoster, onPeerJoin, onPeerMove, onPeerLeave, onChat }) {
  ...
  return {
    ...
    // would delete the new room's row.
    detach: () => {
      if (closed) return;
      closed = true;
