...
export async function joinRoom({
  ...
  // KEEP THE GAME TAB FROM BEING FROZEN (the real background
  // bug). ACTIVE_WINDOW_MS already absorbs Chrome THROTTLING a
  // hidden tab's timers to about one wake-up a minute; what it
  // cannot survive is Chrome FREEZING the tab outright after ~5
  // minutes hidden, because a frozen tab fires no timers at all
  // — zero heartbeats, so the row is pruned and the character
  // vanishes for everyone (and its Realtime socket drops, so a
  // screen it was sharing dies with it). That is the laptop
  // case: start a share, watch the shared window for a few
  // minutes with the game tab hidden, and the broadcaster
  // disappears no matter how wide the prune window is.
  //
  // Holding a Web Lock is one of Chrome's documented reasons a
  // page may NOT be frozen, so we take one for the whole room
  // visit and release it on leave/detach. The tab then stays in
  // the throttled-but-ALIVE tier: the 30s heartbeat keeps
  // landing inside ACTIVE_WINDOW_MS, and a laptop broadcaster
  // staring at the shared window stays visible and on the air.
  // Best-effort and feature-detected — a browser without the
  // Web Locks API just keeps the old behavior. The lock name is
  // unique per connection so a second tab in the same browser
  // never queues behind this one; each visit holds its own lock
  // and is protected on its own.
  ...
  function holdWakeLock() {
    try {
      ...
      const name = `hspn-nb-alive-${player.playerId}-${Math.random()
        .toString(36)
        .slice(2)}`;
      // The request promise stays pending until we resolve it on
      // leave/detach; while it is pending the lock is held, and
      // a held lock is what keeps Chrome from freezing the tab.
      navigator.locks
        .request(
          name,
          { mode: "exclusive" },
          () =>
            new Promise((resolve) => {
              releaseWakeLock = resolve;
            })
        )
        .catch(() => {
          // A rejected request just means no protection this
          // visit — never fatal to the session.
        });
    } catch {
      // No Web Locks here; carry on unprotected.
    }
  }
  function dropWakeLock() {
    try {
      ...
      // already released with the page
    }
  }
  holdWakeLock();
  const joinBody = {
  ...
  if (supabase) {
    ...
    // The big red button (milestone 25). Same trust story as
    // `throw`: only the party route can publish this, so a
    // forged disco is exactly as impossible as a forged kick.
    // Unlike a tomato there is NO optimistic local version —
    // the presser applies the party from the route's own
    // response, so every browser in the room (theirs included)
    // builds the show from the identical id and timestamp and
    // the confetti matches.
    channel.on("broadcast", { event: "party" }, ({ payload }) => {
      if (payload && payload.id && onParty) onParty(payload);
    });
    // The dance button (milestone 26). Same trust story again:
    // only the dance route can publish this, so a forged dance
    // is exactly as impossible as a forged kick. Our OWN dance
    // is applied from the route's HTTP response rather than
    // waiting for this echo, and the echo is then swallowed
    // because that dance is already running — which is also
    // what makes the id (and therefore the routine) identical
    // in every browser, ours included.
    channel.on("broadcast", { event: "dance" }, ({ payload }) => {
      if (payload && payload.id && onDance) onDance(payload);
    });
    // The other half of milestone 27: a dance ENDED. Walking
    // already ends one for everybody off the `move` broadcast,
    // so this is the event for every other way to stop — the
    // toolbar, an overlay, or DANCE pressed a second time while
    // standing perfectly still. Server-published like the rest
    // of this topic, so a forged stop is exactly as impossible
    // as a forged kick.
    channel.on("broadcast", { event: "dance_stop" }, ({ payload }) => {
      if (payload && payload.playerId && onDanceStop) onDanceStop(payload);
    });
    ...
    // HSPN Downs (milestone 24). Same story again: only the
    // horses route can publish this, so the gate opening, the
    // result and the payouts are the truth and there is no
    // optimistic local version of a race. During the betting
    // window the payload carries no seed and no winner — there
    // is nothing in it to peek at.
    channel.on("broadcast", { event: "horses" }, ({ payload }) => {
      if (payload && onHorses) onHorses(payload);
    });
    ...
    // The TV guide (milestone 28). Two events, same trust
    // story as `party` and `dance`: only the channels route
    // can publish them, so a forged channel change is exactly
    // as impossible as a forged kick. `tv_channel` is one
    // validated change (who, what, when, and for a YouTube TV
    // channel the URL the broadcaster's own tab will point its
    // popup at); `tv_mode` says whether that popup broadcast is
    // live, which is what makes the remote appear at all.
    channel.on("broadcast", { event: "tv_channel" }, ({ payload }) => {
      if (payload && payload.channelId && onTvChannel) onTvChannel(payload);
    });
    channel.on("broadcast", { event: "tv_mode" }, ({ payload }) => {
      if (payload && onTvMode) onTvMode(payload);
    });
  ...
  // One beat: renew the Realtime token if it is getting old,
  // say we are still here, and rejoin if the row went away.
  // Pulled out of the interval so the visibility resync below
  // can fire the exact same thing on demand.
  let beating = false;
  async function beat() {
    if (closed || beating) return;
    beating = true;
    try {
      ...
      try {
        ...
        if (r && r.ok === false && r.code === "not_joined") {
          const fresh = await api("join", joinBody);
          if (onRoster) onRoster(fresh.players || [], fresh.serverNow || Date.now());
          // A rejoin makes a brand new row, so the presence
          // entry the room is holding for us is stale. Re-track
          // it or we are in the roster but invisible.
          if (channel) {
            try {
              await channel.track(presencePayload());
            } catch {
              // the reconnect path will re-track anyway
            }
          }
        }
      } catch (err) {
  ...
  // THE VISIBILITY RESYNC. HEARTBEAT_MS is a request, not a
  // promise: Chrome throttles a hidden tab's timers to roughly
  // one wake-up per minute, so a backgrounded player's beats
  // arrive late and irregularly no matter what number we put
  // above. ACTIVE_WINDOW_MS is now wide enough to absorb that,
  // but the moment the tab is looked at again we should stop
  // guessing and go ask the server, because that is the
  // instant a stale roster is most visible to a human.
  //
  // Also covers waking from sleep and a network coming back:
  // both land here as a visibility or online event long before
  // the next throttled interval would have fired.
  function resync() {
    if (closed) return;
    beat();
    // Self-heal a drifted roster the instant a human looks at
    // the tab (or a device wakes / the network returns). A plain
    // heartbeat only rejoins a PRUNED row; it never rebuilds the
    // peer set, so a client that missed a presence "join" — or
    // dropped a peer on a spurious "leave" when the other device
    // slept — would stay half-empty until a full reload (both
    // devices "here" per the count, but not seeing each other).
    // Re-announce ourselves so peers who lost us add us back, and
    // re-apply the authoritative roster so peers we lost come
    // back — exactly the resync the websocket-reconnect path
    // already runs, now also fired by foreground, not only by a
    // socket blip. Best-effort: never blocks or throws into an
    // event handler.
    if (channel) {
      ...
      api("join", joinBody)
        .then((fresh) => {
          if (!closed && onRoster) {
            onRoster(fresh.players || [], fresh.serverNow || Date.now());
          }
        })
        .catch((err) => {
          if (err && err.code === "kicked" && onKicked) {
  ...
  function stopResync() {
    for (const off of resyncEvents.splice(0)) {
      try {
        off();
      } catch {
        // listener already gone with the document
      }
    }
  }
  ...
  return {
    ...
    // Hit the big red button. No arguments at all: the room,
    // the party id and the start timestamp are the server's, so
    // there is nothing here for a client to lie about — and
    // every client in the room derives the same ten seconds of
    // disco from what comes back.
    pressPartyButton: () => api("party", { playerId: player.playerId }),
    // Dance (milestone 26). Like the big red button: no
    // arguments at all. The room, the dance id and the start
    // timestamp are the server's, so there is nothing here for
    // a client to lie about — and every client in the room
    // loops the same footwork from what comes back, for as long
    // as it runs. Works in every room; there is no registry key.
    // A dance ends only when its owner does something else —
    // see stopDance, just below.
    startDance: () =>
      api("dance", { playerId: player.playerId, action: "start" }),
    // Stop dancing. Every deliberate thing you do in the room
    // ends up here, which is why the caller treats it as
    // fire-and-forget: the room drops your dance on the
    // broadcast this produces, and if the request itself is
    // lost, the next roster resync corrects everybody anyway.
    stopDance: () =>
      api("dance", { playerId: player.playerId, action: "stop" }),
    ...
    // Ask the racetrack to do something: "sync" or "bet".
    // Never an outcome — the route already knows which horse
    // wins and is not telling until the gate opens. `sync` is
    // also how the meeting's clock moves, for the same reason
    // the table's does.
    horses: (action, extra) =>
      api("horses", { playerId: player.playerId, action, ...(extra || {}) }),
    // Pull a slot machine's handle (milestone 32). The client
    // sends only its id and the bet — never an outcome. The
    // /slots route rolls the reels and the 1% jackpot with a
    // crypto RNG, moves the money, and returns the result the
    // client must land the reels on. A slot spin is individual
    // and instant, so unlike blackjack and the horses there is
    // nothing broadcast and no clock to sync.
    slots: (action, extra) =>
      api("slots", { playerId: player.playerId, action, ...(extra || {}) }),
    // Change the channel on the big screens (milestone 28).
    // Like the big red button, the client sends no outcome —
    // only which channel it would like. The server decides
    // whether the remote is warm, whether this player is
    // standing in a room that has a TV, and whether anybody is
    // broadcasting YouTube TV to steer; then it broadcasts the
    // change to every room with a screen. The promise carries
    // the route's own answer, so a refusal (a cooldown, say)
    // can be shown as a reason instead of a silence.
    changeChannel: (kind, channelId) =>
      api("channels", {
        playerId: player.playerId,
        action: "change",
        kind,
    ...
    // What is the board showing? Asked on the way in, and
    // after a sleep, so a phone that missed a broadcast heals.
    syncChannel: () =>
      api("channels", { playerId: player.playerId, action: "sync" }),
