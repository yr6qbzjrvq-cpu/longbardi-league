...
function drawSportsBarBackground(ctx, room, theme) {
  ...
  ctx.textBaseline = "middle";
  ctx.fillStyle = P.cream;
  ctx.fillText("88", 39, 200);

  // dartboard, below the jersey
  circle(ctx, 39, 330, 30);
  paint(ctx, P.frame, 2.5);
  circle(ctx, 39, 330, 24);
  paint(ctx, P.cream, 2);
  ...
  for (let i = 0; i < 8; i++) {
    ...
    ctx.beginPath();
    ctx.moveTo(39, 330);
    ctx.lineTo(39 + Math.cos(a) * 24, 330 + Math.sin(a) * 24);
    ctx.stroke();
  }
  circle(ctx, 39, 330, 10);
