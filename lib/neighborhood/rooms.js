// ============================================================
// HSPNeighborhood â room registry (config-as-data)
// ------------------------------------------------------------
...
// Flat fill + soft darker outline â the house style.
...
const TOWN_DARK = {
  ...
  // casino arrow sign (milestone 14) â after dark the bulbs
...
function drawBuilding(ctx, b, P, theme) {
  ...
  // the door itself (tappable â walk over to head inside)
  const dW = 66;
  const dTop = 392;
...
const TOWN_SQUARE = {
  ...
  // east is the casino path. Nothing else moved â every prop,
  // building and exit here is positioned absolutely â and the
  ...
  music: "town", // background loop id â see lib/neighborhood/music.js; no key = silent room
};

// ============================================================
// Interiors (milestone 6) â the three shops off the square.
...
// Milestone 17 â the arcade machine. A house-style upright
// cabinet: navy body, chasing marquee bulbs over a DEEP
// THREAT header, and an attract-mode football looping on the
// little screen. The cabinet is scenery â the GAME is the
...
const FAST_FOOD = {
  ...
  // Milestone 17 â the arcade cabinet. Tap it and your avatar
...
// Milestone 12: TWO rooms have a screen now â the Mission
// Control big board and the Sports Bar's screen over the
// counter â and ONE broadcast lights up both. A client only
...
// The bar's big screen, mounted over the counter â EXACTLY the
...
function drawSportsBarBackground(ctx, room, theme) {
  ...
  // restroom door + sign, back-right corner (milestone 8 â
...
// breathe). Its inner surface IS BAR_SCREEN â the same 16:9
...
function drawBarScreen(ctx, p, P, theme, t) {
  ...
  if (screenFeed !== "live") {
    ...
    ctx.fillStyle = P.cream;
    ctx.fillText(
      "HSPN SPORTS BAR   Â·   GAME OF THE WEEK",
      S.x + S.w / 2,
      S.y + S.h - 44
    );
  }
...
// It used to be painted into the background up on the wall â
...
function drawPartyButton(ctx, p, P, theme, t, fx) {
  ...
  // glow ring under the dome â the "armed" tell
  ctx.save();
...
const SPORTS_BAR = {
  ...
  // accident either â it keeps the whole 608px screen inside a
  ...
  props: [
    ...
    // walkable on purpose â that is the bartender's spot.
    ...
    // Milestone 25 â the big red button, behind the counter
  ...
  // Milestone 25 â the tappable rect over the big red
...
// ============================================================
// Milestone 8 â the secret room chain (Sports Bar easter egg)
// ------------------------------------------------------------
// Restroom â (tap the toilet; the wall it's mounted on spins
// 180Â° into a tunnel mouth) â Hidden Hallway â (steel blast
// door + number pad, code-locked) â Mission Control, a bunker
...
//                CLIENT flag â `type: "reveal"` flips it
...
// client code â theater for a friendly-league easter egg, not
...
// A flat-bottomed brick arch with a dark throat â used for the
...
function drawTunnelMouth(ctx, P, cx, top, bottom, halfW) {
  ...
  ctx.fill();
  // deeper arch â the tunnel recedes
...
// The spinning wall segment â the tunnel cavity behind it is
...
// Tap the toilet (engine flips `tunnelOpen`) and it spins 180Â°
// about its vertical axis â the horizontal scale runs through
...
function drawHallwayBackground(ctx, room, theme) {
  ...
  ctx.fillText("SUB-LEVEL  â¢  AUTHORIZED PERSONNEL ONLY", 440, 70);
...
// The big board's inner surface â EXACTLY 16:9 (608 Ã 342).
...
// and exactly 16:9 â the rect the screen-share <video> is
...
const MISSION_CONTROL = {
  ...
  // the camera follows your feet â so a width-fitted zoom can
...
// ============================================================
// Milestone 21 â the Dairy chain (the secret room behind the
// milk)
// ------------------------------------------------------------
...
//
//   grocery store  â tap the DAIRY cooler; its glass doors
//                    slide apart on a dark opening        (1)
//   Behind the Dairy â a door with an ENORMOUS padlock that
//                    strains, gives up and clunks off     (2)
//   Sub-Dairy Access â a keypad door that accepts literally
//                    any code ("CLOSE ENOUGH")            (3)
//   Restricted Corridor â a palm scanner that thinks hard and
//                    stamps "PROBABLY FINE"               (4)
//   Maximum Security â a bank-vault wheel, steam and all   (5)
//   The Room       â bare walls, one hanging bulb, nothing.
//
// All five rooms are ordinary registry entries: presence,
// chat, tomatoes, voice and moderation work in them exactly
// like anywhere else. None of them set `music` â everything
...
function drawDairyCooler(ctx, p, P, theme, t, fx) {
  ...
  // what is behind the glass â either the chiller's own back
...
function drawKeypadGateDoor(ctx, p, P, theme, t, fx) {
  ...
  ctx.fillText(open ? "OK!" : "â¢â¢â¢â¢", kx, ky + 14);
  for (let r = 0; r < 4; r++) {
    for (let c = 0; c < 3; c++) {
...
function drawScannerDoor(ctx, p, P, theme, t, fx) {
  ...
  if (open && s >= STAMP_AT) {
    ...
    ctx.fillText("PROBABLY FINE â", 0, 1);
    ctx.restore();
  }
}
...
// ---- the end of it -----------------------------------------
// Five locked doors â cooler, padlock, keypad, scanner, vault â
...
function drawGalleryBackground(ctx, room, theme) {
  ...
  // wall â a whisper of a gradient so the room has a top and a
  ...
  // recessed ceiling fixture â the bare bulb's replacement
  ...
  // floor â darker at the back, brighter as it comes forward
  ...
  // contact shadow in the corner â this is the whole reason the
...
const DAIRY_TUNNEL = segment("dairy-tunnel", "Behind the Dairy", {
  ...
  corridor: {
    tile: "tile",
    lamps: [330],
    stencil: "COLD STORAGE  â¢  STAFF ONLY",
...
const DAIRY_KEYPAD = segment("dairy-keypad", "Sub-Dairy Access", {
  ...
  gate: {
    ...
    grantedText: "Close enough â access granted",
  },
...
const DAIRY_VAULT = segment("dairy-vault", "Maximum Security", {
  ...
  corridor: {
    lamps: [230, 400],
    stencil: "MAXIMUM SECURITY  â¢  LEVEL 4",
...
//   casino-floor   the floor itself â patterned carpet, slot
...
//
// The one new config key is `seats` on casino-floor â tappable
// chairs, handled by the room engine the same way `exits` are
// (tap â walk to `approach` â ask the server for the seat).
...
// A proper roadside arrow sign â two posts, a fat rounded
...
// ============================================================
// casino-strip â outdoors, one building
// ============================================================
...
function drawStripMarquee(ctx, p, P, theme, t) {
  ...
  ctx.textAlign = "center";
  ctx.fillStyle = P.trim;
  ctx.fillText("BLACKJACK Â· LOOSE SLOTS Â· NO CLOCKS", p.x, y + 112);
...
const CASINO_STRIP = {
  ...
  // floor (y 596+), and the camera follows your feet â same
...
// ============================================================
// casino-floor â carpet, slots, and the blackjack table
// ============================================================
...
// ============================================================
// The photo frames (milestone 30) â the casino's back wall
// ------------------------------------------------------------
// This wall used to hold the wolf mural (milestone 23). It now
// holds three framed photographs â real photos served as
// static .webp files from /public/neighborhood/. Each frame is
// a chunky house-style gilt border around a portrait thumbnail;
// tap one and the room engine opens a lightbox with the full-
// size image (see PHOTO_FRAMES + the tap handler and
// PhotoOverlay in NeighborhoodRoom.jsx).
//
// The thumbnails are drawn as ordinary depth-sorted PROPS (not
// baked into the cached background) because an <img> loads
// asynchronously: a prop redraws every frame, so the picture
// appears the moment its bytes arrive, with no cache rebuild.
// The Image objects are created lazily and cached module-side,
// and only ever on the client â the draw functions never run in
// the join/moderation API that also imports this file.
//
// To swap a photo later: replace the matching file in
// public/neighborhood/ (keep the portrait ~3:4 shape). The
// *_thumb is what loads on the wall; the *_full is what the
// lightbox shows on tap. Geometry lives in PHOTO_FRAMES below.
// ============================================================

// Each frame's `rect` is its OUTER footprint on the wall (wall
// space, y 0..FLOOR_WALL_Y); the mat + photo are inset by the
// border. Photos are portrait ~1050x1400 (3:4).
const PHOTO_FRAMES = [
  { id: "photo-1", thumb: "/neighborhood/photo1_thumb.webp", full: "/neighborhood/photo1_full.webp", rect: { x: 279, y: 80, w: 120, h: 154 } },
  { id: "photo-2", thumb: "/neighborhood/photo2_thumb.webp", full: "/neighborhood/photo2_full.webp", rect: { x: 421, y: 80, w: 120, h: 154 } },
  { id: "photo-3", thumb: "/neighborhood/photo3_thumb.webp", full: "/neighborhood/photo3_full.webp", rect: { x: 563, y: 80, w: 120, h: 154 } },
  { id: "photo-4", thumb: "/neighborhood/photo4_thumb.webp", full: "/neighborhood/photo4_full.webp", rect: { x: 705, y: 80, w: 120, h: 154 } },
];

// Lazily-created, module-cached <img> objects. Client only: the
// guard means importing this file server-side (the join and
// moderation routes do) never touches the DOM-only Image ctor.
const _photoImgCache = {};
function photoImage(src) {
  if (typeof Image === "undefined") return null;
  let img = _photoImgCache[src];
  if (!img) {
    img = new Image();
    img.decoding = "async";
    img.src = src;
    _photoImgCache[src] = img;
  }
  return img;
}

// One framed photograph on the back wall: a chunky gilt frame
// in the house flat-fill + soft-outline style, a thin cream
// mat, then the photo cover-fitted into the window (or a plain
// placeholder rectangle until the thumbnail loads).
function drawPhotoFrame(ctx, p, P) {
  const r = p.rect;
  const border = 11; // chunky frame
  const mat = 5;     // thin inner mat
  const ix = r.x + border + mat;
  const iy = r.y + border + mat;
  const iw = r.w - (border + mat) * 2;
  const ih = r.h - (border + mat) * 2;

  // cast shadow so the frame reads as hung off the wall
  ctx.save();
  ctx.fillStyle = "rgba(8,4,14,0.4)";
  rr(ctx, r.x + 4, r.y + 6, r.w, r.h, 6);
  ctx.fill();
  ctx.restore();

  // outer gilt frame
  rr(ctx, r.x, r.y, r.w, r.h, 6);
  paint(ctx, P.crown, 2.5);
  // inner bevel line of the frame
  ctx.strokeStyle = shade(P.crown, -0.28);
  ctx.lineWidth = 2;
  rr(ctx, r.x + border, r.y + border, r.w - border * 2, r.h - border * 2, 3);
  ctx.stroke();

  // the mat behind the photo
  ctx.fillStyle = P.cream;
  ctx.fillRect(r.x + border, r.y + border, r.w - border * 2, r.h - border * 2);

  // the photo itself, or a placeholder while it loads
  const img = photoImage(p.thumb);
  if (img && img.complete && img.naturalWidth > 0) {
    ctx.save();
    ctx.beginPath();
    ctx.rect(ix, iy, iw, ih);
    ctx.clip();
    // cover-fit the portrait photo into the (portrait) window
...
function drawFloorBackground(ctx, room, theme) {
  ...
  // neon strip running the full width of the wall. It used to
  // stop short of the milestone-23 wolf mural; the mural is
  // gone (three framed photos hang here now, drawn as props),
  // so the tube light runs end to end again.
  ctx.strokeStyle = P.neon;
  ctx.lineWidth = 6;
  ctx.lineCap = "round";
  ctx.beginPath();
...
// Nothing to play â deliberately: the one game on this floor
...
// â the world view shows the table and who is sitting at it,
...
// the room engine paints their avatar on top â the chair keeps
...
// Milestone 24 â the HSPN Downs betting window, standing on the
...
// â the same walk-then-act shape as a chair, a door and the
// arcade cabinet.
//
...
const CASINO_FLOOR = {
  ...
  // same time as the felt â all of which sit above the floor.
  fitRoom: true,
  spawn: { x: 450, y: 906 },
  ...
  // Milestone 24 â the horse race. Same shape as `arcade` in
  // the Fast Food Place: one hotspot, one approach point, and
  // the room engine does tap â walk â open on the arrival
  ...
  // Milestone 30 â the three framed photos on the back wall.
  // The engine reads these for tap-to-open (the lightbox); the
  // matching props below draw the frames + thumbnails.
  photos: PHOTO_FRAMES,

  props: [
    // The framed photos, hung on the back wall (replacing the
    // wolf mural). sortY is tiny so they draw behind everything
    // else; the async thumbnails appear as soon as they load.
    { id: "photo-1", x: 339, y: 234, sortY: 90, rect: PHOTO_FRAMES[0].rect, thumb: PHOTO_FRAMES[0].thumb, draw: drawPhotoFrame },
    { id: "photo-2", x: 481, y: 234, sortY: 90, rect: PHOTO_FRAMES[1].rect, thumb: PHOTO_FRAMES[1].thumb, draw: drawPhotoFrame },
    { id: "photo-3", x: 623, y: 234, sortY: 90, rect: PHOTO_FRAMES[2].rect, thumb: PHOTO_FRAMES[2].thumb, draw: drawPhotoFrame },
    { id: "photo-4", x: 765, y: 234, sortY: 90, rect: PHOTO_FRAMES[3].rect, thumb: PHOTO_FRAMES[3].thumb, draw: drawPhotoFrame },
...
// a screen open no signaling channel at all â they never had
