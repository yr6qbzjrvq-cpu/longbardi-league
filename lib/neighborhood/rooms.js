...
// ============================================================
// The photo frames (milestone 30) — the casino's back wall
// ------------------------------------------------------------
// This wall used to hold the wolf mural (milestone 23). It now
// holds three framed photographs — real photos served as
...
// and only ever on the client — the draw functions never run in
...
// Milestone 32 — the GRAND PRIZE sign that hangs over each bank
// of slot machines. Austin: "Be sure to advertise that there is
// a grand prize." A lit house-style board, chasing bulbs round
// the edge, Austin's head (the jackpot symbol) x3 and the top
// prize in gold. The head <img> loads lazily through the same
// module-cached loader the wall photos use, so it appears the
// moment its bytes arrive with no background rebuild.
function drawSlotMarquee(ctx, p, P, theme, t) {
  const time = t || 0;
  const w = 200;
  const h = 66;
  const x = p.x - w / 2;
  ...
  // cast shadow so the board reads as hung off the wall
  ctx.save();
  ctx.fillStyle = "rgba(8,4,14,0.4)";
  ...
  // board
  rr(ctx, x, y, w, h, 10);
  paint(ctx, P.neon, 2.5);
  rr(ctx, x + 6, y + 6, w - 12, h - 12, 7);
  paint(ctx, P.slotGlass, 2);
  ...
  ctx.fillText("GRAND PRIZE", p.x, y + 20);

  // the head x3, then the top prize
  const headImg = photoImage("/neighborhood/slot-head.webp");
  const cy = y + 44;
  const startX = p.x - 74;
  for (let i = 0; i < 3; i += 1) {
    ...
    if (headImg && headImg.complete && headImg.naturalWidth > 0) {
      ...
      ctx.clip();
      ctx.drawImage(headImg, hx - 10, cy - 10, 20, 20);
      ctx.restore();
      ctx.strokeStyle = P.slotTrim;
      ctx.lineWidth = 1.4;
      ctx.beginPath();
      ctx.arc(hx, cy, 10, 0, Math.PI * 2);
      ctx.stroke();
  ...
  ctx.fillStyle = P.cream;
  ctx.textAlign = "left";
  ctx.fillText("WIN $500", startX + 3 * 22 + 6, cy);

  marqueeBulbs(ctx, x + 4, y + 4, w - 8, h - 8, 34, time, P, 2.4);
}
...
const CASINO_FLOOR = {
  ...
  // Milestone 32 — the six slot machines are now PLAYABLE. Each
  // is a tap target with the same walk-then-open shape as the
  // arcade cabinet and the betting window: tap a machine, your
  // avatar walks to it, and the slot overlay opens on the
  // arrival frame. All six open the same personal overlay — a
  // spin is individual, against your own wallet, so which
  // machine you stood at does not matter. Geometry mirrors the
  // slot props below (toppers reach ~168px above the feet).
  slots: [
    { id: "slot-1", hotspot: { x: 84, y: 496, w: 72, h: 168 }, approach: { x: 120, y: 700 } },
    { id: "slot-2", hotspot: { x: 176, y: 496, w: 72, h: 168 }, approach: { x: 212, y: 700 } },
    { id: "slot-3", hotspot: { x: 652, y: 496, w: 72, h: 168 }, approach: { x: 688, y: 700 } },
    { id: "slot-4", hotspot: { x: 744, y: 496, w: 72, h: 168 }, approach: { x: 780, y: 700 } },
    { id: "slot-5", hotspot: { x: 60, y: 708, w: 72, h: 168 }, approach: { x: 120, y: 902 } },
    { id: "slot-6", hotspot: { x: 768, y: 708, w: 72, h: 168 }, approach: { x: 780, y: 902 } },
  ],
  ...
  // Milestone 30 — the three framed photos on the back wall.
  ...
  props: [
    ...
    { id: "slot-sign-l", x: 166, y: 486, sortY: 300, draw: drawSlotMarquee },
    { id: "slot-sign-r", x: 734, y: 486, sortY: 300, draw: drawSlotMarquee },
