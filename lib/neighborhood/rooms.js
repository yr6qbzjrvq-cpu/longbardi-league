...
function drawSportsBarBackground(ctx, room, theme) {
  ...
  // dartboard on the wainscot, below the jersey
  circle(ctx, 150, 292, 38);
  paint(ctx, P.frame, 2.5);
  circle(ctx, 150, 292, 30);
  paint(ctx, P.cream, 2);
  ...
  for (let i = 0; i < 8; i++) {
    ...
    ctx.beginPath();
    ctx.moveTo(150, 292);
    ctx.lineTo(150 + Math.cos(a) * 30, 292 + Math.sin(a) * 30);
    ctx.stroke();
  }
  circle(ctx, 150, 292, 12);
  paint(ctx, "#3fae5f", 1.5);
  circle(ctx, 150, 292, 5);
  paint(ctx, "#e2543f", 1.5);

  // restroom door + sign, back-right corner (milestone 8 —
  // the secret chain starts through here)
  rr(ctx, 686, 212, 68, SPORTSBAR_WALL_Y - 212, 8);
  paint(ctx, shade(P.wainscot, -0.14), 3);
  rr(ctx, 696, 224, 48, 58, 5);
  paint(ctx, shade(P.wainscot, 0.1), 2);
  rr(ctx, 696, 290, 48, 34, 5);
  paint(ctx, shade(P.wainscot, 0.1), 2);
  circle(ctx, 744, 276, 4);
  paint(ctx, P.brass, 1.5);
  rr(ctx, 682, 182, 76, 22, 6);
  paint(ctx, P.cream, 2);
  ctx.font = "700 12px Oswald, 'Arial Narrow', sans-serif";
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillStyle = P.ink;
  ctx.fillText("RESTROOM", 720, 194);
...
const SPORTS_BAR = {
  ...
  exits: [
    ...
    },
    {
      ...
      label: "Restroom",
      roomId: "restroom",
      hotspot: { x: 678, y: 178, w: 84, h: 176 },
      approach: { x: 720, y: 384 },
      arrive: { x: 200, y: 680 },
    },
  ],
...
function drawSecretWall(ctx, p, P, theme, t, fx) {
  ...
  if (c >= 0) {
    ...
    if (!spinning) {
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillStyle = "rgba(247,240,220,0.55)";
      ctx.fillText("SHH…", cx, top + 26);
    }
  }
  ctx.restore();
}
  ...
  interact: [
    {
      ...
      toast: "The whole wall rumbles… and spins right around!",
    },
  ],
  ...
  interact: [
    {
      ...
      successToast: "ACCESS GRANTED — the blast door rolls open.",
    },
  ],
...
// Big-board feed state (milestone 9). The <video> element the
// room engine floats over the canvas covers MISSION_SCREEN
// exactly, so once we are "live" the panel underneath only has
// to stop advertising an empty channel. "connecting" is the
// beat between a broadcast starting and the first frame
// arriving. Module-level on purpose: prop draw functions are
// pure (ctx, prop, palette, theme, time) and this is scene
// dressing, not player state.
let missionFeed = "standby"; // standby | connecting | live
export function setMissionScreenState(state) {
  missionFeed = state === "live" || state === "connecting" ? state : "standby";
}
export function getMissionScreenState() {
  return missionFeed;
}
...
function drawBigBoard(ctx, p, P, theme, t) {
  ...
  const on = missionFeed === "live" ? true : Math.floor(t * 1.2) % 2 === 0;
...
// The big board (a prop so the standby art can breathe). ONE
// wall screen, front and center, inner surface = MISSION_SCREEN
// and exactly 16:9. Nothing plays on it yet — standby only.
function drawBigBoard(ctx, p, P, theme, t) {
  const S = MISSION_SCREEN;
  if (theme === "dark") {
    ell(ctx, S.x + S.w / 2, S.y + S.h / 2, S.w * 0.62, S.h * 0.72);
  ...
  ctx.font = "700 18px Oswald, 'Arial Narrow', sans-serif";
  ctx.fillStyle = "rgba(124,196,224," + a.toFixed(3) + ")";
  ctx.fillText("A W A I T I N G   F E E D", S.x + S.w / 2, S.y + S.h / 2 + 22);
