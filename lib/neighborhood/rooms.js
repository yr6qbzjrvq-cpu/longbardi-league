...
// The four runners at HSPN Downs are defined once, in the pure
// race engine, and imported here so the colours on the tote
// board in the world and the colours on the track in the
// overlay cannot drift apart. horses.js is a leaf module with
// no imports of its own, so this stays cheap for the API routes
// that pull in the registry.
import { HORSES } from "@/lib/neighborhood/horses";
...
function drawWelcomeSign(ctx, p, P) {
  ...
  for (const s of [-1, 1]) {
    rr(ctx, p.x + s * 68 - 6, p.y - 96, 12, 96, 5);
    paint(ctx, P.woodDark, 2.5);
  }
  rr(ctx, p.x - 108, p.y - 140, 216, 76, 12);
  paint(ctx, P.wood, 3);
  rr(ctx, p.x - 98, p.y - 130, 196, 56, 8);
  paint(ctx, P.cream, 2);
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillStyle = P.ink;
  ctx.font = "600 12px Inter, system-ui, sans-serif";
  ctx.fillText("WELCOME TO", p.x, p.y - 115);
  ctx.font = "700 21px Oswald, 'Arial Narrow', sans-serif";
  ctx.fillText("HSPNEIGHBORHOOD", p.x, p.y - 91);
}
...
// Behind the counter, at the service end of the bar: a chunky
// steel plinth, a hazard collar, a "DO NOT PRESS" plate that
// nobody has ever obeyed, and a red dome the size of a
// grapefruit. Tapping it walks you over and starts the party
// (see lib/neighborhood/party.js).
//
// Two pieces of live state reach it through the same `fx` bag
// the secret-chain reveals and the tote board use:
//   fx.buttonPress  the rAF-clock time of OUR OWN tap, so the
//                   dome answers the finger instantly instead
//                   of waiting for the server round trip
//   fx.party        the party record every client in the room
//                   is rendering, so the ring is lit for
//                   everybody for exactly as long as the show
const BUTTON_RED = "#e03024";
const BUTTON_RED_DEEP = "#8f1c15";
const BUTTON_RED_LIGHT = "#ff6a55";

function drawPartyButton(ctx, p, P, theme, t, fx) {
  const live = !!(fx && fx.party);
  const pressT = fx && fx.buttonPress ? t - fx.buttonPress : 999;
  // Travel: slams down in 90ms, springs back over the next 330.
  let sink = 0;
  if (pressT >= 0 && pressT < 0.42) {
    sink =
      pressT < 0.09
        ? (pressT / 0.09) * 8
        : 8 * Math.pow(1 - (pressT - 0.09) / 0.33, 2);
  }
  // Idle: a slow breathing glow so the thing asks to be
  // pressed. Live: full blast, on the beat.
  const pulse = live
    ? 0.7 + 0.3 * (0.5 + 0.5 * Math.sin(t * 12.8))
    : 0.32 + 0.28 * (0.5 + 0.5 * Math.sin(t * 2.3));

  softShadow(ctx, p.x, p.y, 38, 12);

  // plinth
  rr(ctx, p.x - 32, p.y - 52, 64, 52, 7);
  paint(ctx, P.steel, 2.5);
  rr(ctx, p.x - 26, p.y - 48, 52, 6, 3);
  paint(ctx, shade(P.steel, 0.22), 1.2);

  // hazard collar
  ctx.save();
  rr(ctx, p.x - 32, p.y - 38, 64, 14, 3);
  ctx.clip();
  ctx.fillStyle = "#f2c81b";
  ctx.fillRect(p.x - 32, p.y - 38, 64, 14);
  ctx.fillStyle = "#241f1a";
  for (let i = -4; i < 5; i++) {
    const sx = p.x + i * 14;
    ctx.beginPath();
    ctx.moveTo(sx, p.y - 38);
    ctx.lineTo(sx + 7, p.y - 38);
    ctx.lineTo(sx - 3, p.y - 24);
    ctx.lineTo(sx - 10, p.y - 24);
    ctx.closePath();
    ctx.fill();
  }
  ctx.restore();
  rr(ctx, p.x - 32, p.y - 38, 64, 14, 3);
  ctx.strokeStyle = shade(P.steel, -0.34);
  ctx.lineWidth = 2;
  ctx.stroke();

  // the plate nobody reads
  rr(ctx, p.x - 27, p.y - 20, 54, 15, 3);
  paint(ctx, P.cream, 1.4);
  ctx.font = "700 8px Oswald, 'Arial Narrow', sans-serif";
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillStyle = P.ink;
  ctx.fillText("DO NOT PRESS", p.x, p.y - 12);

  // the rim the dome sits in
  ell(ctx, p.x, p.y - 52, 34, 12);
  paint(ctx, shade(P.steel, -0.16), 2.5);

  // glow ring under the dome — the "armed" tell
  ctx.save();
  ctx.globalCompositeOperation = "lighter";
  ctx.globalAlpha = pulse * (live ? 0.85 : 0.5);
  const ring = ctx.createRadialGradient(p.x, p.y - 52, 4, p.x, p.y - 52, 54);
  ring.addColorStop(0, "rgba(255,90,70,0.9)");
  ring.addColorStop(1, "rgba(255,90,70,0)");
  ctx.fillStyle = ring;
  ctx.save();
  ctx.translate(p.x, p.y - 52);
  ctx.scale(1, 0.4);
  ctx.beginPath();
  ...
  // the dome: a deep skirt with the bright cap on top
  const dy = p.y - 54 + sink;
  ell(ctx, p.x, dy, 28, 11);
  paint(ctx, BUTTON_RED_DEEP, 2.5);
  ctx.fillStyle = BUTTON_RED_DEEP;
  ctx.fillRect(p.x - 28, dy - 9, 56, 9);
  ell(ctx, p.x, dy - 9, 28, 23);
  paint(ctx, BUTTON_RED, 2.8);
  // gloss
  ctx.globalAlpha = 0.55;
  ell(ctx, p.x - 8, dy - 17, 12, 7);
  ctx.fillStyle = BUTTON_RED_LIGHT;
  ctx.fill();
  ctx.globalAlpha = 0.75;
  ell(ctx, p.x - 10, dy - 19, 5, 3);
...
const SPORTS_BAR = {
  ...
  props: [
    ...
    // Milestone 25 — the big red button, behind the counter
    // at the service end of the bar. The footprint keeps
    // anyone from standing inside it; the walkable lane to
    // its left is how you get behind the bar in the first
    // place, and it is still 76px wide.
    {
      id: "party-button",
      x: 150,
      y: 656,
      draw: drawPartyButton,
      footprint: { x: 116, y: 636, w: 68, h: 22 },
    },
  ],

  // Milestone 25 — the tappable rect over the big red
  // button, plus where you have to be standing to reach it.
  // Exactly the shape the arcade cabinet and the betting
  // window use: tap it, walk there, it fires on arrival.
  partyButton: {
    hotspot: { x: 106, y: 556, w: 88, h: 112 },
    approach: { x: 118, y: 694 },
  },
...
// The exhibit: a plinth, and on it one gallon of chocolate
// milk. The jug is a plain translucent shape on a white plinth
// in a white room, so the chocolate does the talking.
function drawMilkExhibit(ctx, p, P) {
  ...
  // the chocolate milk inside, filled up into the shoulder
  ctx.fillStyle = P.milk;
  ctx.fillRect(x - 40, jy - 84, 80, 86);
  // one gloss stripe down the left shoulder
  ...
  // handle: one simple cut-out
  rr(ctx, x + 10, jy - 72, 22, 34, 9);
...
const DAIRY_SECRET_ROOM = {
  ...
  width: 540,
  height: 540,
  // Off to the left, in front of the door you came through:
  // arriving dead centre put you standing on the price card.
  spawn: { x: 152, y: 470 },
  fitRoom: true,
...
// ============================================================
// The mural (milestone 23) — the casino's back wall
// ------------------------------------------------------------
// Everything else in this world is a flat vector shape with a
// slightly darker outline. This is not that. This is three
// wolves howling at a full moon, airbrushed, and it is utterly
// sincere about it. The gag only lands if the painting really
// is rendered harder than the room it hangs in, so this is the
// one corner of the file that reaches for stacked gradients,
// soft glows, a starfield and several thousand
// individual strokes of fur.
//
// PERFORMANCE CONTRACT: none of this runs per frame. Room
// backgrounds are painted once per (theme, zoom) into an
// offscreen canvas by ensureBg() in the room engine and blitted
// from then on, so the mural costs a few milliseconds at cache
// build time and exactly nothing afterwards. Every value below
// is deterministic — a seeded RNG, no `t` — so a cache rebuild
// repaints the identical picture instead of reshuffling the
// stars.
//
// It is wall art: it lives entirely above FLOOR_WALL_Y, it is
// not a prop and has no footprint, so depth sorting, pathing,
// the table, the slots and the tomatoes are all unaffected. A
// tomato thrown at it still splats on it, which is correct.
// ============================================================

const MURAL = { x: 286, y: 66, w: 590, h: 214, band: 11 };

// Seeded RNG (mulberry32) — the airbrush has to land in the
// same place every repaint.
function muralRng(seed) {
  let s = seed >>> 0;
...
function mixHex(a, b, f) {
  ...
  const b2 = Math.round((A & 255) + ((B & 255) - (A & 255)) * f);
...
// A soft airbrushed blob: radial falloff, squashed vertically.
function wisp(ctx, cx, cy, rx, ry, alpha, tint) {
  const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, rx);
  g.addColorStop(0, `rgba(${tint},${alpha})`);
  g.addColorStop(0.45, `rgba(${tint},${alpha * 0.62})`);
  g.addColorStop(0.78, `rgba(${tint},${alpha * 0.2})`);
...
function muralSky(ctx, A, dim) {
  ...
  for (const [p, c] of stops) g.addColorStop(p, mixHex(c, "#02040a", dim));
  ctx.fillStyle = g;
  ctx.fillRect(A.x, A.y, A.w, A.h);
}

function muralStars(ctx, A, rng, dim) {
  const n = 260;
  for (let i = 0; i < n; i += 1) {
    const x = A.x + rng() * A.w;
    const y = A.y + rng() * A.h * 0.72;
    const depth = 1 - (y - A.y) / (A.h * 0.72); // fewer/dimmer near the horizon haze
    const a = (0.15 + rng() * 0.7) * (0.35 + depth * 0.75) * (1 - dim * 0.25);
    const r = 0.28 + rng() * 0.85;
    ctx.fillStyle = `rgba(${230 + Math.round(rng() * 25)},${238},255,${a.toFixed(3)})`;
    ctx.beginPath();
    ctx.arc(x, y, r, 0, Math.PI * 2);
    ctx.fill();
  }
  // a dozen bright ones with a four-point glare
  for (let i = 0; i < 14; i += 1) {
    const x = A.x + rng() * A.w;
    const y = A.y + rng() * A.h * 0.55;
    const s = 2.2 + rng() * 3.4;
    const g = ctx.createRadialGradient(x, y, 0, x, y, s * 2.6);
    g.addColorStop(0, "rgba(255,255,255,0.85)");
    g.addColorStop(0.3, "rgba(200,226,255,0.35)");
...
// ---- the moon ----------------------------------------------
function muralMoon(ctx, A, mx, my, r, dim) {
  const halo = 1 - dim * 0.3;
  const rings = [
    [r * 5.2, 0.1],
    [r * 3.1, 0.13],
    [r * 2.0, 0.17],
  ...
  for (const [rad, a] of rings) {
    const g = ctx.createRadialGradient(mx, my, r * 0.8, mx, my, rad);
    g.addColorStop(0, `rgba(198,224,255,${(a * halo).toFixed(3)})`);
    g.addColorStop(1, "rgba(198,224,255,0)");
    ctx.fillStyle = g;
    ctx.fillRect(A.x, A.y, A.w, A.h);
  }
  // the disc, lit from the upper left
  const d = ctx.createRadialGradient(mx - r * 0.3, my - r * 0.33, r * 0.06, mx, my, r * 1.02);
  d.addColorStop(0, "#ffffff");
  d.addColorStop(0.4, mixHex("#f4f8ff", "#0a1020", dim * 0.18));
  d.addColorStop(0.78, mixHex("#e6eefb", "#0a1020", dim * 0.22));
  d.addColorStop(1, mixHex("#d3e0f2", "#0a1020", dim * 0.26));
  ctx.save();
  ctx.beginPath();
  ...
  ctx.fill();
  ctx.clip();
  // maria — soft grey seas, no edges anywhere
  const seas = [
    [-0.30, -0.22, 0.36, 0.26, 0.2],
    [0.22, -0.30, 0.26, 0.2, 0.16],
    [0.12, 0.26, 0.42, 0.3, 0.18],
    [-0.38, 0.34, 0.24, 0.18, 0.14],
    [0.44, 0.1, 0.2, 0.24, 0.12],
  ];
  for (const [ox, oy, rx, ry, a] of seas) {
    wisp(ctx, mx + ox * r, my + oy * r, rx * r, ry * r, a, "128,148,180");
  }
  // craters: a bright rim on the moonward side, a soft shadow opposite
  const crRng = muralRng(90210);
  for (let i = 0; i < 22; i += 1) {
    const ang = crRng() * Math.PI * 2;
    const dist = Math.sqrt(crRng()) * r * 0.92;
    const cx = mx + Math.cos(ang) * dist;
    const cy = my + Math.sin(ang) * dist;
    const cr = 1.4 + crRng() * 4.6;
    wisp(ctx, cx + cr * 0.25, cy + cr * 0.25, cr * 1.5, cr * 1.3, 0.22, "116,136,168");
    wisp(ctx, cx - cr * 0.3, cy - cr * 0.3, cr * 1.1, cr * 0.95, 0.2, "255,255,255");
  }
  // limb darkening around the lower right
  const limb = ctx.createRadialGradient(mx - r * 0.25, my - r * 0.25, r * 0.4, mx, my, r);
  limb.addColorStop(0, "rgba(0,0,0,0)");
  limb.addColorStop(1, "rgba(28,44,72,0.24)");
  ctx.fillStyle = limb;
...
// ---- cloud banks drifting across the moon ------------------
function muralClouds(ctx, A, mx, my, rng, dim) {
  const banks = [
    { y: 0.3, x0: 0.02, x1: 0.5, h: 15, a: 0.16 },
    { y: 0.44, x0: 0.28, x1: 0.98, h: 12, a: 0.2 },
    { y: 0.2, x0: 0.55, x1: 1.0, h: 10, a: 0.12 },
    { y: 0.57, x0: 0.0, x1: 0.62, h: 14, a: 0.15 },
  ];
  for (const b of banks) {
    const y0 = A.y + A.h * b.y;
    const n = 26;
    for (let i = 0; i < n; i += 1) {
      const f = i / (n - 1);
      const x = A.x + A.w * (b.x0 + (b.x1 - b.x0) * f);
      const y = y0 + Math.sin(f * 5.2 + b.y * 9) * b.h * 0.5 + (rng() - 0.5) * b.h * 0.8;
      const rx = 16 + rng() * 34;
      const ry = (2.6 + rng() * 4.4) * (b.h / 12);
      // clouds near the moon catch its light
      const near = clamp01(1 - Math.hypot(x - mx, (y - my) * 1.6) / (A.w * 0.34));
      const tint = near > 0.35 ? "214,232,255" : "150,175,208";
      wisp(ctx, x, y, rx, ry, b.a * (0.45 + near * 0.95) * (1 - dim * 0.25), tint);
    }
  }
}

// ---- the far country ---------------------------------------
function ridgePath(ctx, A, baseY, pts) {
  ctx.beginPath();
  ctx.moveTo(A.x - 4, A.y + A.h + 4);
  ctx.lineTo(A.x - 4, baseY + pts[0][1]);
  for (let i = 0; i < pts.length - 1; i += 1) {
    const [x0, y0] = pts[i];
    const [x1, y1] = pts[i + 1];
    const ax = A.x + A.w * x0;
    const bx = A.x + A.w * x1;
    ctx.bezierCurveTo(ax + (bx - ax) * 0.4, baseY + y0, bx - (bx - ax) * 0.4, baseY + y1, bx, baseY + y1);
  }
  ctx.lineTo(A.x + A.w + 4, A.y + A.h + 4);
  ctx.closePath();
}

function muralRidges(ctx, A, mx, my, dim) {
  // far range: hazy, blue, barely there
  const far = [
    [0, 26], [0.14, 6], [0.26, 22], [0.4, 2], [0.52, 18], [0.66, -4], [0.78, 20], [0.9, 8], [1, 24],
  ];
  const farBase = A.y + A.h * 0.6;
  ridgePath(ctx, A, farBase, far);
  const fg = ctx.createLinearGradient(0, farBase - 20, 0, farBase + 60);
  fg.addColorStop(0, mixHex("#4a6d94", "#02040a", dim));
  fg.addColorStop(1, mixHex("#223f5e", "#02040a", dim));
  ctx.fillStyle = fg;
  ctx.fill();
  // moonlight catching the snow line
  ctx.save();
  ridgePath(ctx, A, farBase, far);
  ctx.clip();
  for (const [w, a] of [[9, 0.06], [5, 0.09], [2.4, 0.14], [1.1, 0.22]]) {
    ctx.strokeStyle = `rgba(206,228,252,${a})`;
    ctx.lineWidth = w;
    ridgePath(ctx, A, farBase, far);
    ctx.stroke();
  }
  ...
  const near = [
    [0, 22], [0.1, 4], [0.22, 16], [0.34, 10], [0.46, 24], [0.6, 6], [0.72, 20], [0.86, 2], [1, 18],
  ];
  const nearBase = A.y + A.h * 0.7;
  ridgePath(ctx, A, nearBase, near);
  const ng = ctx.createLinearGradient(0, nearBase - 16, 0, nearBase + 70);
  ng.addColorStop(0, mixHex("#25405f", "#02040a", dim));
  ng.addColorStop(1, mixHex("#0e1c30", "#02040a", dim));
  ctx.fillStyle = ng;
  ctx.fill();
  ctx.save();
  ridgePath(ctx, A, nearBase, near);
  ctx.clip();
  for (const [w, a] of [[7, 0.05], [3.4, 0.09], [1.4, 0.16]]) {
    ctx.strokeStyle = `rgba(186,212,242,${a})`;
    ctx.lineWidth = w;
    ridgePath(ctx, A, nearBase, near);
    ctx.stroke();
  }
...
// ---- the lake, and the moon lying on it --------------------
function muralWater(ctx, A, mx, rng, dim) {
  const top = A.y + A.h * 0.78;
  const g = ctx.createLinearGradient(0, top, 0, A.y + A.h);
  g.addColorStop(0, mixHex("#17304d", "#02040a", dim));
  g.addColorStop(1, mixHex("#060d18", "#02040a", dim));
  ctx.fillStyle = g;
  ctx.fillRect(A.x, top, A.w, A.y + A.h - top);
  // the reflected column, broken into ripples
  for (let i = 0; i < 46; i += 1) {
    const f = i / 45;
    const y = top + f * (A.y + A.h - top);
    const spread = 10 + f * 44;
    const w = spread * (0.35 + rng() * 0.9);
    const x = mx + (rng() - 0.5) * spread * 1.1;
    const a = (0.5 - f * 0.36) * (0.4 + rng() * 0.8) * (1 - dim * 0.3);
    ctx.strokeStyle = `rgba(206,228,255,${Math.max(0, a).toFixed(3)})`;
    ctx.lineWidth = 0.7 + rng() * 1.9;
    ctx.beginPath();
    ctx.moveTo(x - w / 2, y);
    ctx.lineTo(x + w / 2, y);
    ctx.stroke();
  ...
  // shoreline mist
  wisp(ctx, mx, top, A.w * 0.5, 10, 0.22 * (1 - dim * 0.3), "196,218,246");
}

// ---- ground fog --------------------------------------------
function muralMist(ctx, A, rng, dim) {
  const g = ctx.createLinearGradient(0, A.y + A.h * 0.68, 0, A.y + A.h);
  g.addColorStop(0, "rgba(186,210,240,0)");
  g.addColorStop(0.55, `rgba(180,206,238,${(0.1 * (1 - dim * 0.35)).toFixed(3)})`);
  g.addColorStop(1, `rgba(206,226,250,${(0.26 * (1 - dim * 0.35)).toFixed(3)})`);
  ctx.fillStyle = g;
  ctx.fillRect(A.x, A.y + A.h * 0.68, A.w, A.h * 0.32);
  for (let i = 0; i < 16; i += 1) {
    const x = A.x + rng() * A.w;
    const y = A.y + A.h * (0.8 + rng() * 0.2);
    wisp(ctx, x, y, 30 + rng() * 70, 3 + rng() * 7, 0.13 * (1 - dim * 0.3), "212,230,252");
  }
}

// ---- wolves ------------------------------------------------
// Each wolf is an outline in a local 0..100-ish box (feet at
// y=100, facing left) which gets translated, scaled and
// optionally mirrored into place. The outline is flattened to
// a dense polygon first, because the silhouette matters more
// than anything else here: a smooth bezier edge reads as a
// vector shape, so every point along it gets a hair planted
// pointing outward and the coat ends up ragged.

function flattenPath(cmds, steps) {
  const pts = [];
  let cx = 0;
  let cy = 0;
  for (const c of cmds) {
...
// Standing wolf, head thrown back mid-howl, facing left.
const WOLF_BODY = [
  ["M", 2, 2],
  ["C", 9, 0, 16, 4, 22, 11],
  ["C", 25, 12, 27, 12, 30, 13],
  ...
  ["C", 57, 21, 59, 25, 61, 31],
  ["C", 66, 39, 73, 43, 82, 44],
  ["C", 92, 44, 101, 46, 108, 51],
  ["C", 118, 56, 123, 69, 118, 81],
  ["C", 116, 87, 111, 89, 108, 87],
  ["C", 114, 79, 112, 67, 103, 60],
  ["C", 102, 71, 103, 81, 102, 92],
  ["L", 108, 97], ["L", 96, 97],
  ["C", 95, 84, 94, 71, 91, 62],
  ["C", 81, 67, 69, 68, 59, 63],
  ["C", 58, 75, 57, 84, 56, 92],
...
// The foreground animal: head, neck ruff and chest, rising out
// of the fog and running off the bottom of the frame.
const WOLF_BUST = [
  ["M", 2, 18],
  ["C", 12, 9, 24, 5, 35, 9],
  ["C", 42, 12, 47, 17, 50, 22],
  ...
  ["L", 67, 0], ["L", 76, 29],
  ["C", 84, 41, 89, 55, 94, 71],
  ["C", 98, 83, 100, 94, 101, 107],
  ["L", 9, 107],
  ["C", 11, 90, 16, 74, 25, 62],
  ["C", 31, 54, 35, 47, 36, 42],
  ["C", 28, 41, 16, 37, 6, 32],
  ["C", 15, 31, 24, 32.5, 33, 34],
  ["C", 25, 31, 13, 26, 2, 18],
];

function paintWolf(ctx, opts) {
  const { x, y, s, flip, cmds, seed, mx, my, dim, strokes, face, form, origin } = opts;
  const haze = opts.haze || 0;
  const rng = muralRng(seed);
  const pts = flattenPath(cmds, 12);
  ctx.save();
  ctx.translate(x, y);
  ctx.scale(flip ? -s : s, s);
  const lmx = (flip ? -(mx - x) : mx - x) / s;
  const lmy = (my - y) / s;

  const dark = mixHex(mixHex("#0a1220", "#24405f", haze), "#02040a", dim);
  const light = mixHex(mixHex("#e4eefc", "#93b2d2", haze), "#02040a", dim * 0.5);
  const toneAt = (px, py, jitter) => {
    const d = Math.hypot(px - lmx, py - lmy);
    const lit = clamp01(1.15 - d / 105) * 0.62 + clamp01((104 - py) / 104) * 0.38;
    return mixHex(dark, light, clamp01(lit * 0.92 + jitter));
  };

  // a soft cast shadow so it sits in the scene
  wisp(ctx, 60, 99, 56, 8, 0.34, "4,8,16");

  ctx.save();
  tracePoly(ctx, pts);
  ctx.clip();

  const g = ctx.createLinearGradient(0, -6, 0, 108);
  g.addColorStop(0, mixHex(mixHex("#75899f", "#8aa4bd", haze), "#02040a", dim));
  g.addColorStop(0.42, mixHex(mixHex("#3d4e67", "#3f5878", haze), "#02040a", dim));
  g.addColorStop(1, mixHex(mixHex("#111b2b", "#22374f", haze), "#02040a", dim));
  ctx.fillStyle = g;
  ctx.fillRect(-30, -30, 180, 170);

  // form shading: moonlight pooling, belly falling away
  wisp(ctx, lmx, lmy, 136, 136, 0.3 * (1 - dim * 0.3), "196,220,250");
  wisp(ctx, 58, 96, 74, 36, 0.45, "4,8,16");
  wisp(ctx, 22, 74, 42, 44, 0.26, "4,8,16");
  wisp(ctx, origin[0] + 6, origin[1] + 8, 26, 24, 0.2, "206,226,250");

  if (form) form(ctx, dim);

  // The coat, in three passes: broad clumps to establish light
  // and shade, then the hair itself, then the handful of guard
  // hairs the moon actually catches.
  const flow = (px, py) => Math.atan2(py - origin[1], px - origin[0]);
  ctx.lineCap = "round";
  for (let i = 0; i < Math.round(strokes * 0.24); i += 1) {
    const px = -8 + rng() * 140;
    const py = -8 + rng() * 126;
    const a = flow(px, py) + (rng() - 0.5) * 0.42;
    const len = 10 + rng() * 22;
    const bow = (rng() - 0.5) * 5;
    ctx.strokeStyle = toneAt(px, py, (rng() - 0.5) * 0.62);
    ctx.globalAlpha = 0.018 + rng() * 0.055;
    ctx.lineWidth = 3 + rng() * 8;
    ctx.beginPath();
    ctx.moveTo(px, py);
    ctx.quadraticCurveTo(
      px + Math.cos(a) * len * 0.5 - Math.sin(a) * bow,
      py + Math.sin(a) * len * 0.5 + Math.cos(a) * bow,
      px + Math.cos(a) * len,
      py + Math.sin(a) * len,
    );
    ctx.stroke();
  ...
  for (let i = 0; i < strokes; i += 1) {
    const px = -8 + rng() * 140;
    const py = -8 + rng() * 126;
    const a = flow(px, py) + (rng() - 0.5) * 0.85;
    const len = 1.3 + rng() * rng() * 4.6;
    ctx.strokeStyle = toneAt(px, py, (rng() - 0.5) * 0.5);
    ctx.globalAlpha = 0.1 + rng() * 0.36;
    ctx.lineWidth = 0.2 + rng() * 0.45;
    ctx.beginPath();
    ctx.moveTo(px, py);
    ctx.lineTo(px + Math.cos(a) * len, py + Math.sin(a) * len);
    ctx.stroke();
  }
  for (let i = 0; i < Math.round(strokes * 0.45); i += 1) {
    const px = -8 + rng() * 140;
    const py = -8 + rng() * 126;
    const d = Math.hypot(px - lmx, py - lmy);
    const lit = clamp01(1.15 - d / 105) * 0.62 + clamp01((104 - py) / 104) * 0.38;
    if (lit < 0.56) continue;
    const a = flow(px, py) + (rng() - 0.5) * 0.7;
    const len = 1.6 + rng() * rng() * 5.4;
    ctx.strokeStyle = mixHex(light, "#ffffff", rng() * 0.5);
    ctx.globalAlpha = (lit - 0.5) * (0.16 + rng() * 0.4);
    ctx.lineWidth = 0.18 + rng() * 0.42;
    ctx.beginPath();
    ctx.moveTo(px, py);
    ctx.lineTo(px + Math.cos(a) * len, py + Math.sin(a) * len);
    ctx.stroke();
  ...
  // rim light, stacked from wide-and-faint to narrow-and-bright
  // so the edge glows instead of being outlined
  ctx.lineJoin = "round";
  for (const [w, a] of [[11, 0.045], [7, 0.065], [4.4, 0.095], [2.6, 0.14], [1.4, 0.22]]) {
    ctx.strokeStyle = `rgba(206,228,255,${(a * (1 - dim * 0.25) * (1 - haze * 0.4)).toFixed(3)})`;
    ctx.lineWidth = w;
    tracePoly(ctx, pts);
    ctx.stroke();
  }
  // and the edge the moon is actually behind, which is the
  // whole reason anyone paints a wolf in front of a moon
  ctx.lineCap = "round";
  for (const [w, k] of [[6, 0.35], [3, 0.55], [1.5, 0.9]]) {
    ctx.lineWidth = w;
    for (let i = 0; i < pts.length - 1; i += 1) {
      const p = pts[i];
      const d = Math.hypot(p[0] - lmx, p[1] - lmy);
      const lit = clamp01(1.25 - d / 96);
      if (lit < 0.42) continue;
      ctx.strokeStyle = `rgba(232,244,255,${((lit - 0.36) * k * (1 - dim * 0.25) * (1 - haze * 0.45)).toFixed(3)})`;
      ctx.beginPath();
      ctx.moveTo(p[0], p[1]);
  ...
  // ragged coat: hairs planted along the outline, spaced by arc
  // length so the muzzle does not end up hairier than the back
  let sum = 0;
  ...
  const orient = sum > 0 ? 1 : -1;
  let walked = 0;
  for (let i = 0; i < pts.length; i += 1) {
    const p = pts[i];
    const q = pts[(i + 1) % pts.length];
    walked += Math.hypot(q[0] - p[0], q[1] - p[1]);
    if (walked < 1.15) continue;
    walked = 0;
    if (rng() < 0.22) continue;
    const r = pts[(i + 3) % pts.length];
    let nx = r[1] - p[1];
    let ny = -(r[0] - p[0]);
    const l = Math.hypot(nx, ny) || 1;
    nx = (nx / l) * orient;
    ny = (ny / l) * orient;
    const len = 0.9 + rng() * rng() * 4.2;
    const jx = (rng() - 0.5) * 1.4;
    const jy = (rng() - 0.5) * 1.4;
    const bend = (rng() - 0.5) * 0.6;
    ctx.strokeStyle = toneAt(p[0], p[1], 0.1 + (rng() - 0.5) * 0.4);
    ctx.globalAlpha = 0.16 + rng() * 0.44;
    ctx.lineWidth = 0.2 + rng() * 0.4;
    ctx.beginPath();
    ctx.moveTo(p[0] - nx * 2 + jx, p[1] - ny * 2 + jy);
    ctx.lineTo(
      p[0] + jx + (nx * Math.cos(bend) - ny * Math.sin(bend)) * len,
      p[1] + jy + (nx * Math.sin(bend) + ny * Math.cos(bend)) * len,
    );
    ctx.stroke();
  }
  ctx.globalAlpha = 1;

  if (face) face(ctx, rng, dim);
  ctx.restore();
}

function bustForm(ctx, dim) {
  wisp(ctx, 18, 14, 17, 6, 0.24, "206,226,250");   // top plane of the muzzle
  wisp(ctx, 44, 20, 10, 5.5, 0.16, "206,226,250"); // brow ridge
  wisp(ctx, 45, 31, 15, 12, 0.3, "4,8,16");        // cheek falling away
  wisp(ctx, 28, 48, 17, 11, 0.34, "4,8,16");       // under the jaw
  wisp(ctx, 58, 84, 32, 32, 0.26, "4,8,16");       // the heart of the ruff
  wisp(ctx, 88, 66, 18, 28, 0.2, "196,220,250");   // moonlit edge of the ruff
  wisp(ctx, 47, 5, 5, 5, 0.35, "4,8,16");          // dark ear tips
  wisp(ctx, 67, 4, 5, 5, 0.35, "4,8,16");
}

function bodyForm(ctx, dim) {
  wisp(ctx, 12, 6, 11, 4, 0.2, "206,226,250");
  wisp(ctx, 30, 20, 10, 8, 0.24, "4,8,16");
  wisp(ctx, 50, 56, 15, 15, 0.3, "4,8,16");
  wisp(ctx, 82, 46, 26, 8, 0.18, "196,220,250");
  wisp(ctx, 29, 2, 4, 4, 0.32, "4,8,16");
  wisp(ctx, 45, 0, 4, 4, 0.32, "4,8,16");
}

function bodyFace(ctx, rng, dim) {
  // the gape, which the silhouette leaves as a notch
  ctx.fillStyle = "rgba(10,16,26,0.9)";
  ctx.beginPath();
  ctx.moveTo(2, 2);
  ctx.bezierCurveTo(10, 7, 18, 13, 24, 18);
  ctx.bezierCurveTo(18, 19.5, 11, 20.5, 3, 20);
  ctx.closePath();
  ctx.fill();
  ctx.fillStyle = "rgba(88,52,58,0.4)";
  ctx.beginPath();
  ctx.ellipse(13, 14, 5, 1, 0.62, 0, Math.PI * 2);
  ctx.fill();
  // eye
  ctx.fillStyle = mixHex("#dfa839", "#02040a", dim * 0.4);
  ctx.beginPath();
  ctx.ellipse(25.5, 9.6, 1.4, 0.9, -0.5, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(10,14,22,0.9)";
  ctx.beginPath();
  ctx.ellipse(25.6, 9.6, 0.7, 0.95, -0.5, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(255,250,226,0.9)";
  ctx.beginPath();
  ctx.arc(24.9, 9, 0.45, 0, Math.PI * 2);
  ctx.fill();
  // nose
  ctx.fillStyle = "rgba(8,12,20,0.96)";
  ctx.beginPath();
  ctx.ellipse(3, 3, 2, 1.5, -0.55, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(196,220,250,0.5)";
  ctx.beginPath();
  ctx.ellipse(2.3, 2.2, 0.8, 0.5, -0.55, 0, Math.PI * 2);
  ctx.fill();
  // inner ear
  wisp(ctx, 32, 8.6, 2.6, 3.4, 0.4, "44,31,34");
}

function bustFace(ctx, rng, dim) {
  // the gape
  ctx.fillStyle = "rgba(6,10,18,0.95)";
  ctx.beginPath();
  ctx.moveTo(2, 18);
  ctx.bezierCurveTo(13, 26, 25, 31, 33, 34);
  ctx.bezierCurveTo(24, 32.5, 15, 31, 6, 32);
  ctx.closePath();
  ctx.fill();
  // tongue
  ctx.fillStyle = "rgba(104,50,58,0.6)";
  ctx.beginPath();
  ctx.ellipse(18, 29.6, 7, 1.2, 0.5, 0, Math.PI * 2);
  ctx.fill();
  // teeth, upper then lower
  ctx.fillStyle = "rgba(226,236,250,0.62)";
  for (const [tx, ty, tw, dir] of [[5, 21.4, 0.5, 1], [9, 24.4, 0.45, 1], [26.5, 31.4, 0.4, 1], [10, 31.4, 0.45, -1], [15, 31.6, 0.4, -1]]) {
    ctx.beginPath();
    ctx.moveTo(tx, ty);
    ctx.lineTo(tx + tw, ty + 1.5 * dir);
    ctx.lineTo(tx + tw * 2, ty);
    ctx.closePath();
    ctx.fill();
  }
  // nose leather, wet in the moonlight
  ctx.fillStyle = "rgba(8,12,19,0.97)";
  ctx.beginPath();
  ctx.ellipse(3.4, 17, 2.6, 2, -0.62, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(202,226,252,0.62)";
  ctx.beginPath();
  ctx.ellipse(2.5, 15.9, 1, 0.6, -0.62, 0, Math.PI * 2);
  ctx.fill();
  // the eye
  const eg = ctx.createRadialGradient(36, 17, 0.3, 36, 17, 3.8);
  eg.addColorStop(0, "rgba(255,238,186,0.5)");
  eg.addColorStop(0.45, `rgba(226,168,52,${(0.26 * (1 - dim * 0.2)).toFixed(2)})`);
  eg.addColorStop(1, "rgba(226,168,52,0)");
  ctx.fillStyle = eg;
  ctx.beginPath();
  ctx.arc(36, 17, 3.8, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = mixHex("#e0a93a", "#02040a", dim * 0.35);
  ctx.beginPath();
  ctx.ellipse(36, 17, 2, 1.25, -0.46, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(9,13,20,0.95)";
  ctx.beginPath();
  ctx.ellipse(36.2, 17, 0.72, 1, -0.46, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(255,255,255,0.85)";
  ctx.beginPath();
  ctx.arc(35.3, 16.3, 0.5, 0, Math.PI * 2);
  ctx.fill();
  // eyelid and brow, which is what stops it looking like a cartoon
  ctx.strokeStyle = "rgba(8,12,20,0.75)";
  ctx.lineWidth = 0.8;
  ctx.beginPath();
  ctx.moveTo(33.4, 16.4);
  ctx.quadraticCurveTo(36, 14.4, 38.6, 16.6);
  ctx.stroke();
  ctx.strokeStyle = "rgba(214,232,255,0.35)";
  ctx.lineWidth = 0.6;
  ctx.beginPath();
  ctx.moveTo(32.6, 13.8);
  ctx.quadraticCurveTo(37, 11.6, 41.4, 14.6);
  ctx.stroke();
  // a few whiskers, and only a few
  ctx.lineWidth = 0.32;
  ctx.strokeStyle = "rgba(222,236,254,0.35)";
  for (const [wx, wy, ex, ey] of [[8, 20, 1, 27], [9.5, 21.5, 3, 29], [11, 23, 5.5, 30.5]]) {
    ctx.beginPath();
    ctx.moveTo(wx, wy);
    ctx.quadraticCurveTo((wx + ex) / 2, (wy + ey) / 2 + 1.4, ex, ey);
    ctx.stroke();
  }
  // inner ears
  wisp(ctx, 50.5, 15, 4, 5.5, 0.4, "44,31,34");
  wisp(ctx, 65.5, 15.5, 4, 6, 0.4, "44,31,34");
  // lips, drawn as thin lit edges — tightens the gape
  ctx.strokeStyle = "rgba(196,218,246,0.32)";
  ctx.lineWidth = 0.55;
  ctx.beginPath();
  ctx.moveTo(3, 18.4);
  ctx.bezierCurveTo(13, 26, 25, 31, 32.6, 33.4);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(6.6, 31.6);
  ctx.bezierCurveTo(15, 31, 24, 32.4, 32.6, 33.8);
  ctx.stroke();
}

// A boulder for the ridge wolves to stand on, so nobody is
// floating in the fog.
function muralRock(ctx, cx, cy, w, h, seed, dim) {
  const rng = muralRng(seed);
  ctx.save();
  ctx.beginPath();
  ctx.moveTo(cx - w, cy + h * 0.5);
  ctx.bezierCurveTo(cx - w * 0.8, cy - h * 0.5, cx - w * 0.3, cy - h, cx + w * 0.1, cy - h * 0.75);
  ctx.bezierCurveTo(cx + w * 0.6, cy - h * 0.55, cx + w * 0.9, cy - h * 0.1, cx + w, cy + h * 0.5);
  ctx.closePath();
  const g = ctx.createLinearGradient(0, cy - h, 0, cy + h * 0.5);
  g.addColorStop(0, mixHex("#546d8c", "#02040a", dim));
  g.addColorStop(1, mixHex("#0d1826", "#02040a", dim));
  ctx.fillStyle = g;
  ctx.fill();
  ctx.clip();
  for (let i = 0; i < 26; i += 1) {
    const x = cx - w + rng() * w * 2;
    const y = cy - h + rng() * h * 1.5;
    ctx.strokeStyle = `rgba(178,204,236,${(0.05 + rng() * 0.16).toFixed(3)})`;
    ctx.lineWidth = 0.4 + rng() * 0.9;
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.lineTo(x + 3 + rng() * 9, y + (rng() - 0.5) * 3);
    ctx.stroke();
  }
  ctx.restore();
  // moonlight along the top of the stone
  ctx.save();
  ctx.beginPath();
  ctx.moveTo(cx - w, cy + h * 0.5);
  ctx.bezierCurveTo(cx - w * 0.8, cy - h * 0.5, cx - w * 0.3, cy - h, cx + w * 0.1, cy - h * 0.75);
  ctx.bezierCurveTo(cx + w * 0.6, cy - h * 0.55, cx + w * 0.9, cy - h * 0.1, cx + w, cy + h * 0.5);
  ctx.clip();
  for (const [lw, a] of [[5, 0.07], [2.4, 0.12], [1, 0.2]]) {
    ctx.strokeStyle = `rgba(196,220,248,${a})`;
    ctx.lineWidth = lw;
    ctx.beginPath();
    ctx.moveTo(cx - w, cy + h * 0.5);
    ctx.bezierCurveTo(cx - w * 0.8, cy - h * 0.5, cx - w * 0.3, cy - h, cx + w * 0.1, cy - h * 0.75);
...
// ---- the frame ---------------------------------------------
function muralFrame(ctx, M) {
  const b = M.band;
  const g = ctx.createLinearGradient(M.x, M.y, M.x + M.w, M.y + M.h);
  g.addColorStop(0, "#f7e6a2");
  g.addColorStop(0.18, "#c99b2e");
  g.addColorStop(0.38, "#f8eaae");
  g.addColorStop(0.58, "#a87e1c");
  g.addColorStop(0.78, "#efd888");
  g.addColorStop(1, "#b58a22");
  ctx.strokeStyle = g;
  ctx.lineWidth = b;
  ctx.strokeRect(M.x + b / 2, M.y + b / 2, M.w - b, M.h - b);
  ctx.strokeStyle = "rgba(58,40,8,0.8)";
  ctx.lineWidth = 1.3;
  ctx.strokeRect(M.x + 0.5, M.y + 0.5, M.w - 1, M.h - 1);
  ctx.strokeRect(M.x + b - 0.5, M.y + b - 0.5, M.w - b * 2 + 1, M.h - b * 2 + 1);
  ctx.strokeStyle = "rgba(255,246,205,0.5)";
  ctx.lineWidth = 0.9;
  ctx.strokeRect(M.x + 2.5, M.y + 2.5, M.w - 5, M.h - 5);
  ctx.fillStyle = "rgba(255,244,196,0.45)";
  for (let x = M.x + 9; x < M.x + M.w - 6; x += 9) {
    ctx.beginPath();
    ctx.arc(x, M.y + b / 2, 1.4, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(x, M.y + M.h - b / 2, 1.4, 0, Math.PI * 2);
    ctx.fill();
  }
  for (const [cx, cy] of [
    [M.x + b / 2, M.y + b / 2],
    [M.x + M.w - b / 2, M.y + b / 2],
    [M.x + b / 2, M.y + M.h - b / 2],
    [M.x + M.w - b / 2, M.y + M.h - b / 2],
  ]) {
    ctx.fillStyle = "#f7e6a2";
    ctx.beginPath();
    ctx.arc(cx, cy, b * 0.58, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = "rgba(58,40,8,0.7)";
    ctx.lineWidth = 1;
    ctx.stroke();
    ctx.fillStyle = "rgba(120,88,16,0.75)";
    ctx.beginPath();
    ctx.arc(cx, cy, b * 0.2, 0, Math.PI * 2);
    ctx.fill();
  }
}

// The whole picture, painted once into the cached background.
function drawWolfMural(ctx, theme) {
  const M = MURAL;
  const b = M.band;
  const A = { x: M.x + b, y: M.y + b, w: M.w - b * 2, h: M.h - b * 2 };
  const dim = theme === "dark" ? 0.22 : 0;
  const rng = muralRng(20260903);
  const mx = A.x + A.w * 0.5;
  const my = A.y + A.h * 0.31;
  const mr = A.h * 0.33;

  ctx.save();
  ctx.fillStyle = "rgba(12,6,20,0.4)";
  ctx.fillRect(M.x + 5, M.y + 7, M.w, M.h);
  ctx.beginPath();
  ctx.rect(A.x, A.y, A.w, A.h);
  ctx.clip();

  muralSky(ctx, A, dim);
  muralStars(ctx, A, rng, dim);
  muralMoon(ctx, A, mx, my, mr, dim);
  muralClouds(ctx, A, mx, my, rng, dim);
  muralRidges(ctx, A, mx, my, dim);
  muralWater(ctx, A, mx, rng, dim);
  muralMist(ctx, A, rng, dim);

  // The two on the ridge stand on boulders; WOLF_BODY has its
  // feet at local y=97, so the anchor is lifted by that much.
  const sL = A.h * 0.0056;
  const sR = A.h * 0.0051;
  muralRock(ctx, A.x + A.w * 0.12, A.y + A.h * 0.9, 62, 26, 611, dim);
  muralRock(ctx, A.x + A.w * 0.87, A.y + A.h * 0.87, 54, 22, 733, dim);
  paintWolf(ctx, {
    x: A.x + A.w * 0.03, y: A.y + A.h * 0.88 - 16 - 97 * sL, s: sL, flip: false,
    cmds: WOLF_BODY, seed: 7311, mx, my, dim, haze: 0.34, strokes: 1200, face: bodyFace, form: bodyForm, origin: [34, 14],
  });
  paintWolf(ctx, {
    x: A.x + A.w * 0.975, y: A.y + A.h * 0.85 - 14 - 97 * sR, s: sR, flip: true,
    cmds: WOLF_BODY, seed: 9127, mx, my, dim, haze: 0.42, strokes: 1050, face: bodyFace, form: bodyForm, origin: [34, 14],
  });
  paintWolf(ctx, {
    x: A.x + A.w * 0.3, y: A.y + A.h * 0.19, s: A.h * 0.0078, flip: false,
    cmds: WOLF_BUST, seed: 4242, mx, my, dim, strokes: 3400, face: bustFace, form: bustForm, origin: [44, 12],
  });

  // fog drifting back over their feet
  for (let i = 0; i < 12; i += 1) {
    wisp(ctx, A.x + rng() * A.w, A.y + A.h * (0.84 + rng() * 0.18), 34 + rng() * 70, 3 + rng() * 6, 0.11, "216,232,252");
  }

  const v = ctx.createRadialGradient(mx, my + A.h * 0.12, A.h * 0.22, mx, my + A.h * 0.12, A.w * 0.6);
...
function drawFloorBackground(ctx, room, theme) {
  ...
  // neon strip along the wall — it used to run the full width;
  // since milestone 23 it stops short of the mural's frame,
  // with a rounded end, so the painting is not sitting on top
  // of a cut-off tube light.
  ctx.strokeStyle = P.neon;
  ctx.lineWidth = 6;
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.moveTo(24, 214);
  ctx.lineTo(MURAL.x - 18, 214);
  ctx.stroke();
  ctx.strokeStyle = P.neonAlt;
  ctx.lineWidth = 2.5;
  ctx.stroke();
  ...
  // the mural — see drawWolfMural above. Painted over the neon
  // strip and the downlight wash, so it reads as hung on the
  // wall rather than printed onto it.
  drawWolfMural(ctx, theme);
...
// Milestone 24 — the HSPN Downs betting window, standing on the
// casino floor to the left of the blackjack table. A teller
// cage under a tote board: four colours, four names, 4 TO 1
// down the side, and a line at the bottom that counts the next
// race down. Tapping it walks you over and opens the track view
// — the same walk-then-act shape as a chair, a door and the
// arcade cabinet.
//
// The countdown is the ONLY thing in this room that reads live
// state: `fx.race` is handed down from the room engine, which
// got it from the same broadcast the overlay draws from. If it
// is missing (a fresh join, a room with no meeting) the board
// just says HSPN DOWNS and nothing is wrong.
function drawRaceBook(ctx, p, P, theme, t, fx) {
  const time = t || 0;
  const x = p.x;
  const y = p.y;
  softShadow(ctx, x, y, 76, 16);

  // ---- the tote board -------------------------------------
  rr(ctx, x - 70, y - 154, 140, 98, 8);
  paint(ctx, P.wallDark, 2.5);
  rr(ctx, x - 63, y - 147, 126, 84, 5);
  paint(ctx, P.slotGlass, 2);

  // header
  rr(ctx, x - 58, y - 143, 116, 18, 4);
  paint(ctx, P.crown, 1.6);
  ctx.font = "700 12px Oswald, 'Arial Narrow', sans-serif";
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillStyle = P.ink;
  ctx.fillText("HSPN DOWNS", x, y - 134);

  // the four runners
  ctx.font = "700 9px Oswald, 'Arial Narrow', sans-serif";
  for (let i = 0; i < HORSES.length; i += 1) {
    const ry = y - 118 + i * 13;
    circle(ctx, x - 52, ry, 4);
    paint(ctx, HORSES[i].color, 1.2);
    ctx.textAlign = "left";
    ctx.fillStyle = P.cream;
    ctx.fillText(HORSES[i].short, x - 44, ry);
    ctx.textAlign = "right";
    ctx.fillStyle = P.crown;
    ctx.fillText("4:1", x + 56, ry);
  }

  // the live line
  const race = fx && fx.race ? fx.race : null;
  let line = "TAP TO PLAY";
  let tone = P.cream;
  if (race) {
    const secs = Math.max(0, Math.ceil((race.msLeft || 0) / 1000));
    if (race.phase === "running") {
      line = "— OFF AND RUNNING —";
      tone = P.bulbOn;
    } else if (race.phase === "results") {
      const w = HORSES[race.winner];
      line = w ? `${w.short} WINS` : "PHOTO FINISH";
      tone = P.crown;
    } else {
      line = `NEXT RACE  ${secs}s`;
      tone = P.neonAlt;
    }
  }
  ctx.textAlign = "center";
  ctx.font = "700 10px Oswald, 'Arial Narrow', sans-serif";
  ctx.fillStyle = tone;
  ctx.fillText(line, x, y - 69);

  // marquee bulbs around the board, chasing
  marqueeBulbs(ctx, x - 70, y - 154, 140, 98, 28, time, P, 1.5);

  // ---- the counter ----------------------------------------
  rr(ctx, x - 76, y - 60, 152, 12, 4);
  paint(ctx, P.railHi, 2.2);
  rr(ctx, x - 70, y - 50, 140, 50, 6);
  paint(ctx, P.rail, 2.5);

  // teller window with bars, and a slip on the ledge
  rr(ctx, x - 46, y - 44, 92, 28, 4);
  paint(ctx, P.slotGlass, 2);
  ctx.strokeStyle = P.cageBar;
  ctx.lineWidth = 2.4;
  for (let i = 0; i < 4; i += 1) {
    ctx.beginPath();
    ctx.moveTo(x - 34 + i * 23, y - 42);
    ctx.lineTo(x - 34 + i * 23, y - 18);
    ctx.stroke();
  }
  rr(ctx, x + 18, y - 26, 22, 12, 2);
  paint(ctx, P.cream, 1.2);

  ctx.font = "700 9px Oswald, 'Arial Narrow', sans-serif";
  ctx.textAlign = "center";
  ctx.fillStyle = P.matText;
  ctx.fillText("BETTING WINDOW", x, y - 8);

  // a small brass horse on top of the board, because of course
  const bob = Math.sin(time * 1.7) * 1.2;
  ctx.save();
  ctx.translate(x, y - 158 + bob);
  ell(ctx, 0, -8, 13, 6);
  paint(ctx, P.crown, 1.8);
  ctx.beginPath();
  ctx.moveTo(8, -12);
  ctx.lineTo(15, -22);
  ctx.lineTo(20, -19);
  ...
  ctx.beginPath();
  ctx.moveTo(-11, -11);
  ctx.quadraticCurveTo(-20, -14, -22, -4);
  ctx.strokeStyle = shade(P.crown, -0.3);
  ctx.lineWidth = 3;
  ctx.lineCap = "round";
  ctx.stroke();
  for (const lx of [-7, -2, 4, 8]) {
    ctx.beginPath();
    ctx.moveTo(lx, -4);
    ctx.lineTo(lx + (lx > 0 ? 3 : -3), 4);
    ctx.strokeStyle = shade(P.crown, -0.25);
    ctx.lineWidth = 2.4;
    ctx.stroke();
  }
  ctx.restore();
...
const CASINO_FLOOR = {
  ...
  // Milestone 24 — the horse race. Same shape as `arcade` in
  // the Fast Food Place: one hotspot, one approach point, and
  // the room engine does tap → walk → open on the arrival
  // frame. Everything about the racing itself lives in
  // lib/neighborhood/horses.js and the /horses route.
  track: {
    hotspot: { x: 158, y: 700, w: 162, h: 176 },
    approach: { x: 238, y: 902 },
  },
  ...
  props: [
    ...
    { id: "racebook", x: 238, y: 872, sortY: 871, draw: drawRaceBook, footprint: { x: 168, y: 848, w: 140, h: 24 } },
