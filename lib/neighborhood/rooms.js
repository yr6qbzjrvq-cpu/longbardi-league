  ...
  interact: [
    {
      ...
      toast: "The dairy case slides open. That is not a stock room.",
    },
  ],
...
const DAIRY_END_LIGHT = {
  ...
  marker: "rgba(255,255,255,0.8)",
  wall: "#8d8578",
  wallLine: "rgba(0,0,0,0.16)",
  baseboard: "#6e675c",
  floorA: "#7c7568",
  floorLine: "rgba(0,0,0,0.22)",
  steel: "#b6bbc2",
  steelDark: "#7d838b",
  chrome: "#d8dde2",
  tunnel: "#1a1c22",
  tunnelDeep: "#0a0b0e",
  lamp: "#ffe9a8",
  lampGlow: "rgba(255,226,150,0.20)",
  cord: "#3a352c",
  ink: "#241f18",
  cream: "#f4eddc",
  stencil: "rgba(40,34,26,0.30)",
};
...
const DAIRY_END_DARK = {
  ...
  marker: "rgba(255,255,255,0.8)",
  wall: "#575046",
  wallLine: "rgba(0,0,0,0.24)",
  baseboard: "#423c34",
  floorA: "#4a443b",
  floorLine: "rgba(0,0,0,0.30)",
  steel: "#989ea6",
  steelDark: "#666c74",
  chrome: "#b4bac1",
  tunnel: "#111318",
  tunnelDeep: "#050608",
  lamp: "#ffe08e",
  lampGlow: "rgba(255,214,120,0.30)",
  cord: "#241f19",
  ink: "#141109",
  cream: "#e6dcc6",
  stencil: "rgba(240,232,214,0.26)",
};
...
function drawBareRoomBackground(ctx, room, theme) {
  const P = room.palette[theme];
  const wallY = END_WALL_Y;
  ctx.fillStyle = P.wall;
  ctx.fillRect(0, 0, room.width, wallY);
  // faint plaster seams — this is a plain room and it knows it
  ctx.strokeStyle = P.wallLine;
  ctx.lineWidth = 2;
  ...
  // the light pool the bulb throws on the back wall
  if (P.lampGlow) {
    ell(ctx, 268, 190, 150, 130);
    ctx.fillStyle = P.lampGlow;
    ctx.fill();
  }
  rr(ctx, -6, wallY - 12, room.width + 12, 16, 4);
  paint(ctx, P.baseboard, 2);
  ctx.fillStyle = P.floorA;
  ctx.fillRect(0, wallY, room.width, room.height - wallY);
  ctx.strokeStyle = P.floorLine;
  ctx.lineWidth = 2;
  for (let y = wallY + 46; y < room.height; y += 52) {
    ctx.beginPath();
    ctx.moveTo(0, y);
    ctx.lineTo(room.width, y);
    ctx.stroke();
  ...
  if (P.lampGlow) {
    ell(ctx, 268, wallY + 44, 132, 46);
    ctx.fillStyle = P.lampGlow;
    ctx.fill();
  }
  // the round vault doorway back out, standing open
  const B = END_BACK;
  ...
  ctx.fillStyle = P.tunnelDeep;
  ctx.fill();
  ctx.fillStyle = P.lampGlow || "rgba(255,255,255,0.1)";
  ctx.beginPath();
  ctx.moveTo(B.cx - B.r, B.cy + B.r * 0.7);
  ctx.lineTo(B.cx + B.r, B.cy + B.r * 0.7);
  ctx.lineTo(B.cx + B.r + 26, B.cy + B.r + 54);
  ctx.lineTo(B.cx - B.r - 26, B.cy + B.r + 54);
  ctx.closePath();
  ctx.fill();
  drawFrontRim(ctx, room);
...
// One bulb on one cord, swinging very slightly, as if someone
// just walked past it. That is the whole room.
function drawHangingBulb(ctx, p, P, theme, t) {
  const sway = Math.sin(t * 0.7) * 0.045 + Math.sin(t * 1.13) * 0.018;
  const topY = 0;
  const len = 128;
  ctx.save();
  ...
  ctx.stroke();
  // socket
  rr(ctx, -9, len, 18, 20, 4);
  paint(ctx, P.steelDark, 2);
  // glow
  const flick = 0.82 + 0.06 * Math.sin(t * 9.3) + 0.04 * Math.sin(t * 3.1);
  ctx.fillStyle = P.lampGlow || "rgba(255,226,150,0.22)";
  circle(ctx, 0, len + 40, 54 * flick);
  ctx.fill();
  circle(ctx, 0, len + 40, 32 * flick);
  ctx.fill();
  // the bulb: a pear, basically
  ctx.beginPath();
  ctx.moveTo(-8, len + 20);
  ctx.bezierCurveTo(-24, len + 30, -22, len + 60, 0, len + 60);
  ctx.bezierCurveTo(22, len + 60, 24, len + 30, 8, len + 20);
  ctx.closePath();
  ...
  ctx.stroke();
  // filament
  ctx.strokeStyle = "rgba(180,120,20,0.8)";
  ctx.lineWidth = 1.6;
  ctx.beginPath();
  ctx.moveTo(-5, len + 34);
  for (let i = 0; i < 5; i++) {
    ctx.lineTo(-5 + i * 2.6, len + 34 + (i % 2 ? 8 : 0));
  }
  ctx.lineTo(5, len + 34);
  ctx.stroke();
  ctx.restore();
...
const DAIRY_TUNNEL = segment("dairy-tunnel", "Behind the Dairy", {
  ...
  gate: {
    ...
    flag: "dairyPadlock",
    hotspot: GATE_HOTSPOT,
    toast: "The padlock strains, thinks better of it, and hits the floor.",
  },
...
const DAIRY_KEYPAD = segment("dairy-keypad", "Sub-Dairy Access", {
  ...
  gate: {
    ...
    successToast: "The door does not check. The door has never checked.",
  },
...
const DAIRY_SCANNER = segment("dairy-scanner", "Restricted Corridor", {
  ...
  gate: {
    ...
    flag: "dairyScanner",
    hotspot: GATE_HOTSPOT,
    toast: "Scanning… analyzing… PROBABLY FINE ✓",
  },
...
const DAIRY_VAULT = segment("dairy-vault", "Maximum Security", {
  ...
  gate: {
    ...
    flag: "dairyVault",
    hotspot: GATE_HOTSPOT,
    toast: "Bolts retract. Steam everywhere. Something opens.",
  },
  gateDraw: drawVaultWheelDoor,
});

// The payoff. Deliberately, gloriously nothing — one bulb and
// four walls. Austin gets to decide later what belongs here;
// until then the joke is the emptiness.
const DAIRY_SECRET_ROOM = {
  ...
  props: [{ id: "bulb", x: 268, y: 190, sortY: 296, draw: drawHangingBulb }],

  drawBackground: drawBareRoomBackground,
};
