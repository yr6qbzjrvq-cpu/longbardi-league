...
import { POPUP_HEARTBEAT_MS } from "@/lib/neighborhood/channels";
...
export function createBroadcaster({ conn, selfId, roomId, onState, onViewers }) {
  ...
  // ---- popup mode (milestone 28) ---------------------------
  // The room's remote only works when what is on the board is
  // a window THIS TAB opened: an opener may set the location
  // of a window it opened, cross-origin included, and that is
  // the entire trick. A plain screen share of some other tab
  ...
  // Tell the server the popup broadcast is live (or over).
  // The commissioner cookie is what authorizes it, and a stamp
  // goes stale on its own, so a shut laptop takes the remote
  // away without anybody having to press anything.
  async function stampPopup(state, quiet) {
    try {
      await fetch("/api/neighborhood/broadcast", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          playerId: selfId,
      ...
      // a missed stamp costs one heartbeat, not the session
    }
  }

  // Re-stamp the instant the game tab is looked at again.
  // While a popup broadcast is running that tab is hidden by
  // definition — Austin is watching the popup — so its timers
  // are throttled to roughly one wake-up a minute and the
  // stamp lands late. POPUP_LIVE_WINDOW_MS is wide enough to
  // absorb that now, but coming back to the tab is the one
  // moment we can be exact instead of patient, so take it.
  let popupVisSync = null;
  function watchPopupVisibility() {
    if (popupVisSync || typeof document === "undefined") return;
    popupVisSync = () => {
      if (document.visibilityState !== "visible") return;
      if (!live) return;
      if (popupAlive()) stampPopup("live", true);
    };
    document.addEventListener("visibilitychange", popupVisSync);
  }
  function unwatchPopupVisibility() {
    if (!popupVisSync || typeof document === "undefined") return;
    document.removeEventListener("visibilitychange", popupVisSync);
    popupVisSync = null;
  }
  ...
  function watchPopup() {
    ...
    popupTimer = setInterval(() => {
      if (!live) return;
      if (!popupAlive()) {
        // He closed the window the board was watching. There
        // is nothing left to capture and nothing to steer, so
        // end the broadcast rather than leave a dead rectangle
        // hanging on two walls.
        stop();
        return;
      }
      stampPopup("live", true);
    }, POPUP_HEARTBEAT_MS);
  }

  // Point the popup at a channel. Returns false when the
  // window is gone, which is the room's cue to say so out
  // loud instead of pretending the channel changed.
  function tuneTo(url) {
    if (!popupAlive() || !url) return false;
    try {
      popup.location.href = url;
      return true;
    } catch {
      // Navigating a window we opened is allowed even
      // cross-origin; anything throwing here means we do not
      // own that window any more.
      return false;
    }
  }
  ...
  // one standing in front of the big board; the room roster is
  // checked too, for a transport with no board presence.
  //
  // PRESENCE IS A HINT, NOT A GATE. It was a hard requirement
  // until the milestone-12 live test, where every hello was
  // refused because this deployment's Realtime reports no
  // presence at all (a client/server version thing — the same
  // emptiness shows up on the gameplay topic). Nothing about
  // trust depends on it: being on the screen channel is what
  // makes someone reachable, and what makes an offer BELIEVED
  // is the signed, nonce-bound grant the viewer verifies
  // server-side. So when presence tells us nothing, answer the
  // hello; when it tells us something, honour it.
  function watching(id) {
    if (!id || id === selfId) return false;
    try {
      const screen = conn.screenPresenceIds ? conn.screenPresenceIds() : [];
      const room = conn.presenceIds ? conn.presenceIds() : [];
      if (screen.includes(id) || room.includes(id)) return true;
      return screen.length === 0 && room.length === 0;
    } catch {
      return true;
    }
  }
  ...
  async function start(options) {
    ...
    popup = (options && options.popup) || null;
    ...
    if (popup) {
      // Mark the remote live BEFORE anybody in the room can
      // reach for it, then keep the stamp warm.
      await stampPopup("live", false);
      watchPopup();
    }
    report();
  }
  ...
  function stop() {
    ...
    if (popup) {
      // The window itself stays open — he may still want to
      // watch it. Only the room's remote goes away.
      stampPopup("idle", false);
      popup = null;
    }
    if (conn && conn.signal) {
