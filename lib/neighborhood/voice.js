...
export function createVoiceMesh({
  ...
  let distSource = null; // engine's id → world-px distance lookup
  ...
  function vadTick() {
    // Volumes ride this tick too, not just the rAF loop: a
    // hidden tab freezes requestAnimationFrame entirely, but
    // an interval only slows to ~1Hz — so someone with the
    // tab in the background keeps hearing people at the right
    // loudness as everyone walks (positions stay computable
    // because they are deterministic path+timestamp math).
    if (distSource) applyDistances(distSource);
    const now = Date.now();
    const next = new Set();
  ...
  // walking never zips or clicks. The lookup is KEPT: the VAD
  // tick re-applies it between calls, which is what keeps
  // volumes moving while a hidden tab's rAF is frozen.
  function updateDistances(distFor) {
    if (typeof distFor === "function") distSource = distFor;
    applyDistances(distSource);
  }

  function applyDistances(distFor) {
    if (!live || typeof distFor !== "function") return;
    const ctx = voiceCtx;
    for (const [id, e] of peers) {
      const d = distFor(id);
