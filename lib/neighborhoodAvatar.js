...
//
// Milestone 27 loops this. A dance runs until its owner does
// something else, so drawAvatar wraps "seconds since the dance
// started" into the cycle and hands the remainder down here:
// the caller repeats, and the choreography stays a pure
// function of ONE time through. The neutral tail of each cycle
// is the little breath between repeats.
//
// Four beats, then a breath, then again:
//
 ...
 *   dance   — { t, cycleMs, style } (milestones 26, 27): t is
 *             seconds since the dance STARTED. A dance runs
 *             until its owner does something else, so this can
 *             be minutes; it is wrapped into cycleMs below and
 *             the routine repeats. Present = this
...
export function drawAvatar(ctx, config, x, y, scale = 1, opts = {}) {
  ...
  //
  // milestone 27: `t` is seconds since the dance STARTED, and a
  // dance now runs until its owner does something else, so it
  // can be minutes. One cycle of the routine is cycleMs long and
  // it simply repeats: wrapping here keeps danceRig a pure
  // function of ONE time through, and because the wrap is plain
  // arithmetic on one shared start timestamp, four phones
  // watching the same dancer are on the same beat of the same
  // cycle without a byte of extra traffic.
  const danceDur =
    opts.dance && Number(opts.dance.cycleMs) > 400
      ? Number(opts.dance.cycleMs) / 1000
      : 3.6;
  const danceT =
    opts.dance && typeof opts.dance.t === "number" && opts.dance.t >= 0
      ? opts.dance.t % danceDur
      : -1;
  const d =
    danceT >= 0 ? danceRig(danceT, danceDur, opts.dance.style || 0) : null;
  ...
  if (d) {
    ...
    drawNotes(ctx, danceT, danceDur, opts.dance.style || 0, headY - headR, torsoW * 0.55);
  }
