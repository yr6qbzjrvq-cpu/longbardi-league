...
// Word-by-word version of the keep-it-friendly filter for
// longer text (chat, milestone 5). Flattening a whole sentence
// would invent words across boundaries ("grass hit" flattens
// to "grasshit"), so each word is flattened and checked on its
// own. Pure — shared by the chat API route and the client.
export function containsBlockedWord(text) {
  return String(text || "")
    .split(/[^A-Za-z0-9@$]+/)
    .some((piece) => {
      const flat = flatten(piece);
      return flat && BLOCKED.some((word) => flat.includes(word));
    });
}
